...
std::string NvapiStatusString(NvAPI_Status status) {
  ...
  if (NvAPI_GetErrorMessage(status, err) == NVAPI_OK) { return std::string(err); }
  return "NVAPI_ERROR";
}
...
bool ParseSrcDevicePair(const char *text, NvU32 *srcId, NvU32 *device) {
  if (!text || !srcId || !device) { return false; }
...
const SubcommandEntry *FindSubcommand(const SubcommandEntry *entries, size_t count, const char *name) {
  if (!entries || count == 0 || !name) { return nullptr; }
...
double KBToMiB(NvU32 kb) { return static_cast<double>(kb) / 1024.0; }

double BytesToMiB(NvU64 bytes) { return static_cast<double>(bytes) / (1024.0 * 1024.0); }

const char *BusTypeName(NV_GPU_BUS_TYPE type) {
  switch (type) {
  case NVAPI_GPU_BUS_TYPE_PCI: return "PCI";
  case NVAPI_GPU_BUS_TYPE_AGP: return "AGP";
  case NVAPI_GPU_BUS_TYPE_PCI_EXPRESS: return "PCIe";
  case NVAPI_GPU_BUS_TYPE_FPCI: return "FPCI";
  case NVAPI_GPU_BUS_TYPE_AXI: return "AXI";
...
const char *RamTypeName(NV_GPU_RAM_TYPE type) {
  switch (type) {
  case NV_GPU_RAM_TYPE_SDRAM: return "SDRAM";
  case NV_GPU_RAM_TYPE_DDR1: return "DDR1";
  case NV_GPU_RAM_TYPE_DDR2: return "DDR2";
  case NV_GPU_RAM_TYPE_GDDR2: return "GDDR2";
  case NV_GPU_RAM_TYPE_GDDR3: return "GDDR3";
  case NV_GPU_RAM_TYPE_GDDR4: return "GDDR4";
  case NV_GPU_RAM_TYPE_DDR3: return "DDR3";
  case NV_GPU_RAM_TYPE_GDDR5: return "GDDR5";
  case NV_GPU_RAM_TYPE_LPDDR2: return "LPDDR2";
  case NV_GPU_RAM_TYPE_GDDR5X: return "GDDR5X";
  case NV_GPU_RAM_TYPE_HBM1: return "HBM1";
  case NV_GPU_RAM_TYPE_HBM2: return "HBM2";
  case NV_GPU_RAM_TYPE_SDDR4: return "SDDR4";
  case NV_GPU_RAM_TYPE_GDDR6: return "GDDR6";
  case NV_GPU_RAM_TYPE_GDDR6X: return "GDDR6X";
  default: return "UNKNOWN";
  }
}
...
const char *UtilizationDomainName(NvU32 domain) {
  switch (domain) {
  case NVAPI_GPU_UTILIZATION_DOMAIN_GPU: return "GPU";
  case NVAPI_GPU_UTILIZATION_DOMAIN_FB: return "FB";
  case NVAPI_GPU_UTILIZATION_DOMAIN_VID: return "VID";
  case NVAPI_GPU_UTILIZATION_DOMAIN_BUS: return "BUS";
...
const char *ClockDomainName(NvU32 domain) {
  switch (domain) {
  case NVAPI_GPU_PUBLIC_CLOCK_GRAPHICS: return "GRAPHICS";
  case NVAPI_GPU_PUBLIC_CLOCK_MEMORY: return "MEMORY";
  case NVAPI_GPU_PUBLIC_CLOCK_PROCESSOR: return "PROCESSOR";
  case NVAPI_GPU_PUBLIC_CLOCK_VIDEO: return "VIDEO";
  default: return "OTHER";
  }
}
...
const char *PstateClockTypeName(NV_GPU_PERF_PSTATE20_CLOCK_TYPE_ID type) {
  switch (type) {
  case NVAPI_GPU_PERF_PSTATE20_CLOCK_TYPE_SINGLE: return "SINGLE";
  case NVAPI_GPU_PERF_PSTATE20_CLOCK_TYPE_RANGE: return "RANGE";
  default: return "UNKNOWN";
  }
}
...
const char *ConnectorTypeName(NV_MONITOR_CONN_TYPE type) {
  switch (type) {
  case NV_MONITOR_CONN_TYPE_VGA: return "VGA";
  case NV_MONITOR_CONN_TYPE_COMPONENT: return "COMPONENT";
  case NV_MONITOR_CONN_TYPE_SVIDEO: return "SVIDEO";
  case NV_MONITOR_CONN_TYPE_HDMI: return "HDMI";
  case NV_MONITOR_CONN_TYPE_DVI: return "DVI";
  case NV_MONITOR_CONN_TYPE_LVDS: return "LVDS";
  case NV_MONITOR_CONN_TYPE_DP: return "DP";
  case NV_MONITOR_CONN_TYPE_COMPOSITE: return "COMPOSITE";
  case NV_MONITOR_CONN_TYPE_UNINITIALIZED: return "UNINITIALIZED";
  default: return "UNKNOWN";
  }
}
...
const char *PstateName(NV_GPU_PERF_PSTATE_ID pstate) {
  switch (pstate) {
  case NVAPI_GPU_PERF_PSTATE_P0: return "P0";
  case NVAPI_GPU_PERF_PSTATE_P1: return "P1";
  case NVAPI_GPU_PERF_PSTATE_P2: return "P2";
  case NVAPI_GPU_PERF_PSTATE_P3: return "P3";
  case NVAPI_GPU_PERF_PSTATE_P4: return "P4";
  case NVAPI_GPU_PERF_PSTATE_P5: return "P5";
  case NVAPI_GPU_PERF_PSTATE_P6: return "P6";
  case NVAPI_GPU_PERF_PSTATE_P7: return "P7";
  case NVAPI_GPU_PERF_PSTATE_P8: return "P8";
  case NVAPI_GPU_PERF_PSTATE_P9: return "P9";
  case NVAPI_GPU_PERF_PSTATE_P10: return "P10";
  case NVAPI_GPU_PERF_PSTATE_P11: return "P11";
  case NVAPI_GPU_PERF_PSTATE_P12: return "P12";
  case NVAPI_GPU_PERF_PSTATE_P13: return "P13";
  case NVAPI_GPU_PERF_PSTATE_P14: return "P14";
  case NVAPI_GPU_PERF_PSTATE_P15: return "P15";
  default: return "UNKNOWN";
  }
}
...
const char *CoolerControlName(NV_COOLER_CONTROL control) {
  switch (control) {
  case NVAPI_COOLER_CONTROL_TOGGLE: return "TOGGLE";
  case NVAPI_COOLER_CONTROL_VARIABLE: return "VARIABLE";
  default: return "NONE";
  }
}
...
const char *VoltageDomainName(NV_GPU_PERF_VOLTAGE_INFO_DOMAIN_ID domain) {
  switch (domain) {
  case NVAPI_GPU_PERF_VOLTAGE_INFO_DOMAIN_CORE: return "CORE";
  default: return "UNKNOWN";
  }
}
...
const char *PerfVoltageDomainName(NV_GPU_PERF_VOLTAGE_DOMAIN_ID domain) {
  switch (domain) {
  case NVAPI_GPU_PERF_VOLTAGE_DOMAIN_CORE: return "CORE";
  case NVAPI_GPU_PERF_VOLTAGE_DOMAIN_FB: return "FB";
  case NVAPI_GPU_PERF_VOLTAGE_DOMAIN_COLD_CORE: return "COLD_CORE";
  case NVAPI_GPU_PERF_VOLTAGE_DOMAIN_CORE_NOMINAL: return "CORE_NOMINAL";
  case NVAPI_GPU_PERF_VOLTAGE_DOMAIN_UNDEFINED: return "UNDEFINED";
  default: return "UNKNOWN";
  }
}
...
const char *VoltageEntryTypeName(NV_GPU_PSTATE20_VOLTAGE_ENTRY_TYPE type) {
  switch (type) {
  case NV_GPU_PSTATE20_VOLTAGE_ENTRY_TYPE_LOGICAL: return "LOGICAL";
  case NV_GPU_PSTATE20_VOLTAGE_ENTRY_TYPE_VDT: return "VDT";
  case NV_GPU_PSTATE20_VOLTAGE_ENTRY_TYPE_DELTA_ONLY: return "DELTA_ONLY";
  case NV_GPU_PSTATE30_VOLTAGE_ENTRY_TYPE_VFE: return "VFE";
  case NV_GPU_PSTATE30_VOLTAGE_ENTRY_TYPE_PSTATE: return "PSTATE";
  case NV_GPU_PSTATE30_VOLTAGE_ENTRY_TYPE_VPSTATE: return "VPSTATE";
  case NV_GPU_PSTATE30_VOLTAGE_ENTRY_TYPE_FREQUENCY: return "FREQUENCY";
  case NV_GPU_PSTATE20_VOLTAGE_ENTRY_TYPE_UNDEFINED: return "UNDEFINED";
  default: return "UNKNOWN";
  }
}
...
const char *ClockDomainIdName(NV_GPU_CLOCK_DOMAIN_ID domain) {
  switch (domain) {
  case NVAPI_GPU_CLOCK_DOMAIN_GPC: return "GPC";
  case NVAPI_GPU_CLOCK_DOMAIN_XBAR: return "XBAR";
  case NVAPI_GPU_CLOCK_DOMAIN_SYS: return "SYS";
  case NVAPI_GPU_CLOCK_DOMAIN_HUB: return "HUB";
  case NVAPI_GPU_CLOCK_DOMAIN_M: return "M";
  case NVAPI_GPU_CLOCK_DOMAIN_HOST: return "HOST";
  case NVAPI_GPU_CLOCK_DOMAIN_DISP: return "DISP";
  case NVAPI_GPU_CLOCK_DOMAIN_HOTCLK: return "HOTCLK";
  case NVAPI_GPU_CLOCK_DOMAIN_PCLK0: return "PCLK0";
  case NVAPI_GPU_CLOCK_DOMAIN_PCLK1: return "PCLK1";
  case NVAPI_GPU_CLOCK_DOMAIN_BYPCLK: return "BYPCLK";
  case NVAPI_GPU_CLOCK_DOMAIN_XCLK: return "XCLK";
  case NVAPI_GPU_CLOCK_DOMAIN_VPV: return "VPV";
  case NVAPI_GPU_CLOCK_DOMAIN_VPS: return "VPS";
  case NVAPI_GPU_CLOCK_DOMAIN_GPUCACHECLK: return "GPUCACHECLK";
  case NVAPI_GPU_CLOCK_DOMAIN_UNDEFINED: return "UNDEFINED";
  default: return "UNKNOWN";
  }
}
...
const char *ClockPstateUsageName(NV_GPU_CLOCK_INFO_DOMAIN_PSTATE_USAGE usage) {
  switch (usage) {
  case NV_GPU_CLOCK_PSTATE_USAGE_FIXED: return "FIXED";
  case NV_GPU_CLOCK_PSTATE_USAGE_PSTATE: return "PSTATE";
  case NV_GPU_CLOCK_PSTATE_USAGE_DECOUPLED: return "DECOUPLED";
  case NV_GPU_CLOCK_PSTATE_USAGE_RATIO: return "RATIO";
  default: return "UNKNOWN";
  }
}
...
const char *PerfLimitInputTypeName(NV_GPU_PERF_LIMIT_STATUS_INPUT_TYPE type) {
  switch (type) {
  case NVAPI_PERF_LIMIT_STATUS_INPUT_TYPE_DISABLED: return "DISABLED";
  case NVAPI_PERF_LIMIT_STATUS_INPUT_TYPE_PSTATE: return "PSTATE";
  case NVAPI_PERF_LIMIT_STATUS_INPUT_TYPE_FREQ: return "FREQ";
  case NVAPI_PERF_LIMIT_STATUS_INPUT_TYPE_VPSTATE: return "VPSTATE";
  case NVAPI_PERF_LIMIT_STATUS_INPUT_TYPE_VOLTAGE: return "VOLTAGE";
  case NVAPI_PERF_LIMIT_STATUS_INPUT_TYPE_VOLTAGE_EX: return "VOLTAGE_EX";
  case NVAPI_PERF_LIMIT_STATUS_INPUT_TYPE_VOLTAGE_3X: return "VOLTAGE_3X";
  default: return "UNKNOWN";
  }
}
...
const char *PerfLimitPstatePointName(NV_GPU_PERF_LIMIT_INPUT_DATA_PSTATE_POINT point) {
  switch (point) {
  case NV_GPU_PERF_LIMIT_INPUT_DATA_PSTATE_POINT_NOM: return "NOM";
  case NV_GPU_PERF_LIMIT_INPUT_DATA_PSTATE_POINT_MIN: return "MIN";
  case NV_GPU_PERF_LIMIT_INPUT_DATA_PSTATE_POINT_MAX: return "MAX";
  case NV_GPU_PERF_LIMIT_INPUT_DATA_PSTATE_POINT_MID: return "MID";
  default: return "UNKNOWN";
  }
}
...
const char *VpstateTypeName(NV_GPU_PERF_VPSTATE_TYPE type) {
  switch (type) {
  case NV_GPU_PERF_VPSTATE_TYPE_2X: return "2X";
  case NV_GPU_PERF_VPSTATE_TYPE_3X: return "3X";
  default: return "UNKNOWN";
  }
}
...
const char *VfeVarTypeName(NV_GPU_PERF_VFE_VAR_TYPE type) {
  switch (type) {
  case NV_GPU_PERF_VFE_VAR_TYPE_DERIVED: return "DERIVED";
  case NV_GPU_PERF_VFE_VAR_TYPE_DERIVED_PRODUCT: return "DERIVED_PRODUCT";
  case NV_GPU_PERF_VFE_VAR_TYPE_DERIVED_SUM: return "DERIVED_SUM";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE: return "SINGLE";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE_FREQUENCY: return "SINGLE_FREQUENCY";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE_SENSED: return "SINGLE_SENSED";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE: return "SINGLE_SENSED_FUSE";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE_SENSED_TEMP: return "SINGLE_SENSED_TEMP";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE_VOLTAGE: return "SINGLE_VOLTAGE";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_BASE: return "SINGLE_SENSED_FUSE_BASE";
  case NV_GPU_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_20: return "SINGLE_SENSED_FUSE_20";
  default: return "UNKNOWN";
  }
}
...
const char *VfeVarOverrideTypeName(NvU8 type) {
  switch (type) {
  case NV_GPU_PERF_VFE_VAR_SINGLE_OVERRIDE_TYPE_NONE: return "NONE";
  case NV_GPU_PERF_VFE_VAR_SINGLE_OVERRIDE_TYPE_VALUE: return "VALUE";
  case NV_GPU_PERF_VFE_VAR_SINGLE_OVERRIDE_TYPE_OFFSET: return "OFFSET";
  case NV_GPU_PERF_VFE_VAR_SINGLE_OVERRIDE_TYPE_SCALE: return "SCALE";
  default: return "UNKNOWN";
  }
}
...
const char *VfeEquTypeName(NV_GPU_PERF_VFE_EQU_TYPE type) {
  switch (type) {
  case NV_GPU_PERF_VFE_EQU_TYPE_COMPARE: return "COMPARE";
  case NV_GPU_PERF_VFE_EQU_TYPE_MINMAX: return "MINMAX";
  case NV_GPU_PERF_VFE_EQU_TYPE_QUADRATIC: return "QUADRATIC";
  case NV_GPU_PERF_VFE_EQU_TYPE_EQUATION_SCALAR: return "EQUATION_SCALAR";
  default: return "UNKNOWN";
  }
}
...
const char *VfeEquOutputTypeName(NvU8 type) {
  switch (type) {
  case NV_GPU_PERF_VFE_EQU_OUTPUT_TYPE_UNITLESS: return "UNITLESS";
  case NV_GPU_PERF_VFE_EQU_OUTPUT_TYPE_FREQ_MHZ: return "FREQ_MHZ";
  case NV_GPU_PERF_VFE_EQU_OUTPUT_TYPE_VOLT_UV: return "VOLT_UV";
  case NV_GPU_PERF_VFE_EQU_OUTPUT_TYPE_VF_GAIN: return "VF_GAIN";
  case NV_GPU_PERF_VFE_EQU_OUTPUT_TYPE_VOLT_DELTA_UV: return "VOLT_DELTA_UV";
  default: return "UNKNOWN";
  }
}
...
const char *VfeEquCompareFuncName(NvU8 func) {
  switch (func) {
  case NV_GPU_PERF_VFE_EQU_COMPARE_FUNCTION_EQUAL: return "EQ";
  case NV_GPU_PERF_VFE_EQU_COMPARE_FUNCTION_GREATER_EQ: return "GTE";
  case NV_GPU_PERF_VFE_EQU_COMPARE_FUNCTION_GREATER: return "GT";
  default: return "UNKNOWN";
  }
}

bool ParseGpuIndexArgs(int argc, char **argv, NvU32 *indexOut, bool *hasIndex) {
  if (!indexOut || !hasIndex) { return false; }
  *hasIndex = false;
...
bool ParseDisplayIdsArgs(int argc, char **argv, NvU32 *gpuIndex, bool *hasIndex, bool *all, NvU32 *flags) {
  if (!gpuIndex || !hasIndex || !all || !flags) { return false; }
  *hasIndex = false;
  *all = false;
  *flags = 0;
  for (int i = 0; i < argc; ++i) {
...
bool ParsePstateId(const char *value, NV_GPU_PERF_PSTATE_ID *out) {
  ...
  if ((value[0] == 'P' || value[0] == 'p') && value[1] != '\0') {
    NvU32 numeric = 0;
    if (!ParseUint(value + 1, &numeric)) { return false; }
    if (numeric > NVAPI_GPU_PERF_PSTATE_P15) { return false; }
...
bool Utf8ToNvUnicode(const char *input, NvAPI_UnicodeString out) {
  ...
  if (needed <= 0 || needed > static_cast<int>(NVAPI_UNICODE_STRING_MAX)) { return false; }
  std::vector<wchar_t> buffer(static_cast<size_t>(needed));
  if (!MultiByteToWideChar(CP_UTF8, 0, input, -1, buffer.data(), needed)) { return false; }
...
std::string NvUnicodeToUtf8(const NvAPI_UnicodeString value) {
  ...
  size_t length = 0;
  while (length < NVAPI_UNICODE_STRING_MAX && wide[length] != L'\0') { ++length; }
...
std::string GuidToString(const NvGUID &guid) {
  char buffer[64] = {};
  std::snprintf(buffer, sizeof(buffer), "{%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X}", guid.data1, guid.data2,
...
const char *TargetViewModeName(NV_TARGET_VIEW_MODE mode) {
  switch (mode) {
  case NV_VIEW_MODE_STANDARD: return "STANDARD";
  case NV_VIEW_MODE_CLONE: return "CLONE";
  case NV_VIEW_MODE_HSPAN: return "HSPAN";
  case NV_VIEW_MODE_VSPAN: return "VSPAN";
  case NV_VIEW_MODE_DUALVIEW: return "DUALVIEW";
  case NV_VIEW_MODE_MULTIVIEW: return "MULTIVIEW";
  default: return "UNKNOWN";
  }
}
...
const char *MosaicTopoTypeName(NV_MOSAIC_TOPO_TYPE type) {
  switch (type) {
  case NV_MOSAIC_TOPO_TYPE_ALL: return "ALL";
  case NV_MOSAIC_TOPO_TYPE_BASIC: return "BASIC";
  case NV_MOSAIC_TOPO_TYPE_PASSIVE_STEREO: return "PASSIVE_STEREO";
  case NV_MOSAIC_TOPO_TYPE_SCALED_CLONE: return "SCALED_CLONE";
  case NV_MOSAIC_TOPO_TYPE_PASSIVE_STEREO_SCALED_CLONE: return "PASSIVE_STEREO_SCALED_CLONE";
  default: return "UNKNOWN";
  }
}
...
const char *MosaicTopoName(NV_MOSAIC_TOPO topo) {
  switch (topo) {
  case NV_MOSAIC_TOPO_NONE: return "NONE";
  case NV_MOSAIC_TOPO_1x2_BASIC: return "1x2_BASIC";
  case NV_MOSAIC_TOPO_2x1_BASIC: return "2x1_BASIC";
  case NV_MOSAIC_TOPO_1x3_BASIC: return "1x3_BASIC";
  case NV_MOSAIC_TOPO_3x1_BASIC: return "3x1_BASIC";
  case NV_MOSAIC_TOPO_1x4_BASIC: return "1x4_BASIC";
  case NV_MOSAIC_TOPO_4x1_BASIC: return "4x1_BASIC";
  case NV_MOSAIC_TOPO_2x2_BASIC: return "2x2_BASIC";
  case NV_MOSAIC_TOPO_2x3_BASIC: return "2x3_BASIC";
  case NV_MOSAIC_TOPO_2x4_BASIC: return "2x4_BASIC";
  case NV_MOSAIC_TOPO_3x2_BASIC: return "3x2_BASIC";
  case NV_MOSAIC_TOPO_4x2_BASIC: return "4x2_BASIC";
  case NV_MOSAIC_TOPO_1x5_BASIC: return "1x5_BASIC";
  case NV_MOSAIC_TOPO_1x6_BASIC: return "1x6_BASIC";
  case NV_MOSAIC_TOPO_7x1_BASIC: return "7x1_BASIC";
  case NV_MOSAIC_TOPO_1x2_PASSIVE_STEREO: return "1x2_PASSIVE_STEREO";
  case NV_MOSAIC_TOPO_2x1_PASSIVE_STEREO: return "2x1_PASSIVE_STEREO";
  case NV_MOSAIC_TOPO_1x3_PASSIVE_STEREO: return "1x3_PASSIVE_STEREO";
  case NV_MOSAIC_TOPO_3x1_PASSIVE_STEREO: return "3x1_PASSIVE_STEREO";
  case NV_MOSAIC_TOPO_1x4_PASSIVE_STEREO: return "1x4_PASSIVE_STEREO";
  case NV_MOSAIC_TOPO_4x1_PASSIVE_STEREO: return "4x1_PASSIVE_STEREO";
  case NV_MOSAIC_TOPO_2x2_PASSIVE_STEREO: return "2x2_PASSIVE_STEREO";
  default: return "UNKNOWN";
  }
}
...
const char *GsyncDisplaySyncStateName(NVAPI_GSYNC_DISPLAY_SYNC_STATE state) {
  switch (state) {
  case NVAPI_GSYNC_DISPLAY_SYNC_STATE_UNSYNCED: return "UNSYNCED";
  case NVAPI_GSYNC_DISPLAY_SYNC_STATE_SLAVE: return "SLAVE";
  case NVAPI_GSYNC_DISPLAY_SYNC_STATE_MASTER: return "MASTER";
  default: return "UNKNOWN";
  }
}
...
const char *DpNodeTypeName(NV_DP_NODE_TYPE type) {
  switch (type) {
  case NV_DP_NODE_TYPE_DP: return "DP";
  case NV_DP_NODE_TYPE_HDMI: return "HDMI";
  case NV_DP_NODE_TYPE_DVI: return "DVI";
  case NV_DP_NODE_TYPE_VGA: return "VGA";
...
bool MaskE32Has(const NV_GPU_BOARDOBJGRP_MASK_E32 &mask, NvU32 index) {
  if (index >= NV_GPU_BOARDOBJGRP_E32_MAX_OBJECTS) { return false; }
...
bool MaskE255Has(const NV_GPU_BOARDOBJGRP_MASK_E255 &mask, NvU32 index) {
  if (index >= NV_GPU_BOARDOBJGRP_E255_MAX_OBJECTS) { return false; }
  const NvU32 word = index / 32;
  const NvU32 bit = index % 32;
  const NvU32 *data = mask.super.pData;
  return (data[word] & (1u << bit)) != 0;
...
void PrintUsageGpu() {
  ...
  std::printf("  %s gpu pstates20-set [--index N] --pstate P0 (--clock graphics|memory|processor|video --delta-khz N | "
              "--voltage core --delta-uv N)\n",
              kToolName);
  std::printf("  %s gpu pstates20-private [--index N]\n", kToolName);
  std::printf("  %s gpu pstates20-private-set [--index N] --pstate P0 (--clock-id ID --delta-khz N | --voltage-domain "
              "core|fb|cold-core|core-nominal|ID (--delta-uv N | --target-uv N))\n",
              kToolName);
  std::printf("  %s gpu bus [--index N]\n", kToolName);
  std::printf("  %s gpu vbios [--index N]\n", kToolName);
  std::printf("  %s gpu cooler [--index N]\n", kToolName);
  std::printf("  %s gpu cooler-policy get [--index N] --cooler N [--policy "
              "manual|perf|temp-discrete|temp-cont|temp-cont-sw|default]\n",
              kToolName);
  std::printf("  %s gpu cooler-policy set [--index N] --cooler N --level-id N --level PCT [--policy "
              "manual|perf|temp-discrete|temp-cont|temp-cont-sw|default]\n",
              kToolName);
  std::printf("  %s gpu cooler-policy restore [--index N] [--cooler N] [--policy "
              "manual|perf|temp-discrete|temp-cont|temp-cont-sw|default]\n",
              kToolName);
  ...
  std::printf("  %s gpu vf inject [--index N] [--flags HEX] [--clk-domain ID --clk-khz N] [--volt-domain "
              "logic|sram|msvdd|ID --volt-rail N --volt-uv N --volt-min-uv N]\n",
              kToolName);
  std::printf("  %s gpu vpstates info [--index N]\n", kToolName);
  std::printf("  %s gpu vpstates control [--index N] [--original]\n", kToolName);
  std::printf("  %s gpu vpstates set [--index N] --vpstate N (--clock N --target-mhz N [--min-eff-mhz N] | --group N "
              "--value N)\n",
              kToolName);
  std::printf("  %s gpu vfe-var info [--index N]\n", kToolName);
  std::printf("  %s gpu vfe-var control [--index N]\n", kToolName);
  std::printf("  %s gpu vfe-var set [--index N] --var N --override-type none|value|offset|scale [--override-value F] "
              "[--temp-hyst-pos C] [--temp-hyst-neg C]\n",
              kToolName);
  std::printf("  %s gpu vfe-equ info [--index N]\n", kToolName);
  std::printf("  %s gpu vfe-equ control [--index N]\n", kToolName);
  std::printf("  %s gpu vfe-equ set [--index N] --equ N (--compare-func eq|gte|gt --compare-crit F | --minmax min|max "
              "| --coeffs A,B,C)\n",
              kToolName);
  std::printf("  %s gpu perf-limits info [--index N]\n", kToolName);
  std::printf("  %s gpu perf-limits status [--index N]\n", kToolName);
  std::printf("  %s gpu perf-limits set [--index N] --limit-id ID --type disabled|pstate|freq|vpstate [--pstate P0] "
              "[--point nominal|min|max|mid] [--freq-khz N --domain ID] [--vpstate N]\n",
              kToolName);
  ...
  std::printf("  %s gpu fan set [--index N] --cooler N --level PCT [--policy "
              "manual|perf|temp-discrete|temp-cont|temp-cont-sw|default]\n",
              kToolName);
  ...
  std::printf(
      "  %s gpu client-illum zones set [--index N] --zone N --mode "
      "manual-rgb|manual-rgbw|manual-single|manual-color-fixed --brightness N [--r N --g N --b N --w N] [--default]\n",
      kToolName);
  std::printf("\n");
}
...
void PrintUsageDisplay() {
  ...
  std::printf("  %s display custom try --id HEX --width W --height H --refresh R [--depth BPP] [--type "
              "auto|cvt|cvt-rb|gtf|dmt|dmt-rb|eia861|analog-tv|nv-predefined] [--interlaced 0|1] [--hw-only 0|1] "
  ...
  std::printf("  %s display bpc set --id HEX [--bpc default|6|8|10|12|16] [--dither 0|1] [--force-link 0|1] "
              "[--force-rg-div 0|1]\n",
              kToolName);
  ...
  std::printf("  %s display hdr color set --id HEX [--mode MODE] [--format FORMAT] [--range RANGE] [--bpc BPC] "
              "[--os-hdr default|on|off]\n",
              kToolName);
...
void PrintUsageMosaic() {
  ...
  std::printf(
      "  %s mosaic supported [--type all|basic|passive-stereo|scaled-clone|passive-stereo-scaled-clone] [--limit N]\n",
      kToolName);
...
void PrintUsageGsync() {
  ...
  std::printf("  %s gsync sync set --index N --display-id HEX --state master|slave|unsynced [--no-validate] "
              "[--send-start-event]\n",
              kToolName);
  ...
  std::printf(
      "  %s gsync control set --index N [--polarity rising|falling|both] [--video-mode none|ttl|ntsc|hdtv|composite] "
...
void PrintUsageVideo() {
  ...
  std::printf("  %s video color set [--handle-index N] [--brightness N] [--contrast N] [--hue N] [--saturation N] "
              "[--color-temp N] [--ygamma N] [--rgamma N] [--ggamma N] [--bgamma N] [--override|--use-app]\n",
              kToolName);
  std::printf("\n");
}
...
void PrintUsageHdmi() {
  ...
  std::printf(
      "  %s hdmi stereo modes --id HEX [--start N] [--count N] [--pass-through] [--width W --height H --refresh R]\n",
      kToolName);
...
void PrintUsageDp() {
  ...
  std::printf("  %s dp set [--id HEX] [--handle-index N] [--output-id HEX] [--link-rate RATE] [--lane-count N] "
...
void PrintUsage() { PrintUsageSummary(); }
...
void PrintDrsSetting(const NVDRS_SETTING &setting) {
  ...
  std::printf("  0x%08X %s (%s) ", setting.settingId, name.empty() ? "<unnamed>" : name.c_str(),
              DrsSettingTypeName(setting.settingType));

  switch (setting.settingType) {
  case NVDRS_DWORD_TYPE: std::printf("value=0x%08X (%u)", setting.u32CurrentValue, setting.u32CurrentValue); break;
  ...
  case NVDRS_BINARY_TYPE: {
    ...
    for (NvU32 i = 0; i < dumpCount; ++i) { std::printf("%02X", setting.binaryCurrentValue.valueData[i]); }
    if (length > maxDump) { std::printf("..."); }
    break;
  }
  default: std::printf("value=<unsupported>"); break;
  }
  std::printf("\n");
}

bool GetDrsProfileByName(NvDRSSessionHandle session, const char *name, NvDRSProfileHandle *outProfile) {
  if (!name || !outProfile) { return false; }
