...
const char *EccConfigName(NV_ECC_CONFIGURATION config) {
  switch (config) {
  case NV_ECC_CONFIGURATION_NOT_SUPPORTED: return "NOT_SUPPORTED";
  case NV_ECC_CONFIGURATION_DEFERRED: return "DEFERRED";
  case NV_ECC_CONFIGURATION_IMMEDIATE: return "IMMEDIATE";
...
const char *PcieAspmStateName(NV_PCIE_ASPM_STATE state) {
  switch (state) {
  case NV_PCIE_ASPM_STATE_DISABLED: return "DISABLED";
  case NV_PCIE_ASPM_STATE_L0S: return "L0S";
  case NV_PCIE_ASPM_STATE_L1: return "L1";
  case NV_PCIE_ASPM_STATE_L0S_L1: return "L0S_L1";
  default: return "UNKNOWN";
  }
}
...
const char *PcieGenerationName(NV_PCIE_GENERATION gen) {
  switch (gen) {
  case NV_PCIE_GENERATION_GEN_1: return "GEN1";
  case NV_PCIE_GENERATION_GEN_2: return "GEN2";
  case NV_PCIE_GENERATION_GEN_3: return "GEN3";
  case NV_PCIE_GENERATION_GEN_4: return "GEN4";
  case NV_PCIE_GENERATION_GEN_NONE: return "NONE";
  default: return "UNKNOWN";
  }
}
...
const char *SurpriseRemovalBusName(NV_KNOWN_SURPRISE_REMOVAL_BUS_TYPE type) {
  switch (type) {
  case NV_KNOWN_SURPRISE_REMOVAL_BUS_TYPE_UNKNOWN: return "UNKNOWN";
  case NV_KNOWN_SURPRISE_REMOVAL_BUS_TYPE_TB2: return "TB2";
  case NV_KNOWN_SURPRISE_REMOVAL_BUS_TYPE_TB3: return "TB3";
  case NV_KNOWN_SURPRISE_REMOVAL_BUS_TYPE_OEM_SPECIFIC: return "OEM";
  default: return "UNKNOWN";
  }
}
...
std::string BufferToAscii(const NvU8 *data, size_t size, bool *hasNonPrintable) {
  if (hasNonPrintable) { *hasNonPrintable = false; }
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if (std::isprint(ch)) {
      ...
      if (hasNonPrintable) { *hasNonPrintable = true; }
      out.push_back('.');
    }
  }
  return out;
...
void PrintPcieLinkInfo(const char *label, const NV_PCIE_LINK_INFO_V2 &info) {
  std::printf("  %s:\n", label);
  std::printf("    maxSpeed=%u Mbps maxWidth=%u gen=%s aspm=%s\n", info.maxSpeed, info.maxWidth,
              PcieGenerationName(info.maxGeneration), PcieAspmStateName(info.supportedStates));
  std::printf("    currentSpeed=%u Mbps currentWidth=%u gen=%s aspm=%s\n", info.currentSpeed, info.currentWidth,
              PcieGenerationName(info.currentGeneration), PcieAspmStateName(info.currentState));
}
...
int CmdGpuEccSet(int argc, char **argv) {
  ...
  for (size_t i = 0; i < handles.size(); ++i) {
    ...
    if (status != NVAPI_OK) {
      ...
      if (status == NVAPI_INVALID_USER_PRIVILEGE) { std::printf("  Try running the terminal as Administrator.\n"); }
      continue;
    }
...
int CmdGpuEcc(int argc, char **argv) {
  ...
  if (std::strcmp(argv[0], "status") == 0) { return CmdGpuEccStatus(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "errors") == 0) { return CmdGpuEccErrors(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "config") == 0) { return CmdGpuEccConfig(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "set") == 0) { return CmdGpuEccSet(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "reset") == 0) { return CmdGpuEccReset(argc - 1, argv + 1); }
...
int CmdGpuBoardInfo(int argc, char **argv) {
  ...
  for (size_t i = 0; i < handles.size(); ++i) {
    ...
    if (status == NVAPI_OK) { std::printf("  GPU serial: 0x%016llX\n", static_cast<unsigned long long>(serial)); }
...
int CmdGpuBoard(int argc, char **argv) {
  ...
  if (std::strcmp(argv[0], "mfg") == 0) { return CmdGpuBoardManufacturing(argc - 1, argv + 1); }
...
int CmdGpuPcieInfo(int argc, char **argv) {
  ...
  for (size_t i = 0; i < handles.size(); ++i) {
    ...
    std::printf("  Surprise removal: vendor=0x%04X device=0x%04X supported=%s bus=%s\n",
                info.rootPortSurpriseRemovalCaps.vendorId, info.rootPortSurpriseRemovalCaps.deviceId,
...
int CmdGpuPcie(int argc, char **argv) {
  ...
  if (std::strcmp(argv[0], "info") == 0) { return CmdGpuPcieInfo(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "switch-errors") == 0) { return CmdGpuPcieSwitchErrors(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "errors") == 0) { return CmdGpuPcieErrors(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "aer") == 0) { return CmdGpuPcieAer(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "aspm-set") == 0) { return CmdGpuPcieAspmSet(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "width-set") == 0) { return CmdGpuPcieSetWidth(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "speed-set") == 0) { return CmdGpuPcieSetSpeed(argc - 1, argv + 1); }
