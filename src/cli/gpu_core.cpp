...
namespace nvcli
{
    namespace
    {
        const char *CoolerPolicyName(NV_COOLER_POLICY policy)
        {
            switch (policy)
            {
            case NVAPI_COOLER_POLICY_MANUAL:
                return "MANUAL";
            case NVAPI_COOLER_POLICY_PERF:
                return "PERF";
            case NVAPI_COOLER_POLICY_TEMPERATURE_DISCRETE:
                return "TEMP_DISCRETE";
            case NVAPI_COOLER_POLICY_TEMPERATURE_CONTINUOUS:
                return "TEMP_CONT";
            case NVAPI_COOLER_POLICY_TEMPERATURE_CONTINUOUS_SW:
                return "TEMP_CONT_SW";
            case NVAPI_COOLER_POLICY_DEFAULT:
                return "DEFAULT";
            default:
                return "UNKNOWN";
            }
        ...
        for (size_t i = 0; i < handles.size(); ++i)
        {
            ...
            if (status == NVAPI_OK)
            {
                ...
                for (NvU32 d = 0; d < domainsInfo.numDomains; ++d)
                {
                    ...
                    std::printf("    %s(%u): step=%u uV\n",
                        VoltageDomainName(domain.domainId),
                        domain.domainId,
                        domain.stepSizeuV);
                }
            }
            ...
            if (status == NVAPI_OK)
            {
                ...
                for (NvU32 d = 0; d < domainsStatus.numDomains; ++d)
                {
                    ...
                    std::printf("    %s(%u): %u uV (%.3f mV)\n",
                        ...
                        domain.domainId,
                        domain.voltageuV,
                        static_cast<double>(domain.voltageuV) / 1000.0);
                }
            }
            else
            {
            ...
            if (status == NVAPI_OK)
            {
                ...
                for (NvU32 d = 0; d < voltages.numDomains; ++d)
                {
                    ...
                    for (NvU32 v = 0; v < limit; ++v)
                    {
                        std::printf("      [%u] %u mV\n", v, domain.voltages[v].mvolt);
                    }
                    if (domain.numVoltages > limit)
                    {
    ...
    int CmdGpuCoolerPolicy(int argc, char **argv)
    {
        ...
        {
            return CmdGpuCoolerPolicyGet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "set") == 0)
        {
            return CmdGpuCoolerPolicySet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "restore") == 0)
        {
            return CmdGpuCoolerPolicyRestore(argc - 1, argv + 1);
        }
