...
bool CreateSwapChain(D3DContext &ctx, DebugLog *debugLog) {
  ...
  if (!GetFactoryFromDevice(ctx, &factory)) { return false; }
...
bool ParseAnselFeatureSpec(const char *text, NVAPI_ANSEL_FEATURE_CONFIGURATION_STRUCT *out) {
  ...
  if (!key.empty()) {
    NvU32 hotkey = 0;
    if (!ParseUint(key.c_str(), &hotkey)) { return false; }
...
bool SafeD3DSetVRRState(IUnknown *deviceOrContext, NVDX_ObjectHandle surfaceHandle, BOOL enable,
                        NvAPI_Status *statusOut) {
...
bool SafeD3DGetVRRState(IUnknown *deviceOrContext, NVDX_ObjectHandle surfaceHandle, BOOL *isEnabled, BOOL *isRequested,
                        NvAPI_Status *statusOut) {
  if (!statusOut) { return false; }
  NvAPI_Status status = NVAPI_ERROR;
  __try {
...
int CmdD3dVrr(int argc, char **argv) {
  ...
  if (debug) {
    if (fopen_s(&debugLog.file, "nvapi-cli_d3d_vrr.log", "a") == 0 && debugLog.file) { debugLog.Log("----"); }
  }
  ...
  NvAPI_Status status = NVAPI_OK;
  IUnknown *deviceOrContext = ctx.context ? static_cast<IUnknown *>(ctx.context) : static_cast<IUnknown *>(ctx.device);
  if (doSet) {
...
int CmdD3dLatency(int argc, char **argv) {
  ...
  if (std::strcmp(argv[0], "get") == 0) {
    ...
    if (raw) { DumpHex(&params, sizeof(params)); }
...
int CmdD3d(int argc, char **argv) {
  ...
  return DispatchSubcommand("d3d", argc, argv, kSubcommands, sizeof(kSubcommands) / sizeof(kSubcommands[0]),
                            PrintD3dUsage);
}
} // namespace nvcli
