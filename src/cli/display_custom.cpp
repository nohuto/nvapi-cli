...
namespace nvcli
{
    namespace
    {
        ...
        void PrintCustomDisplay(const NV_CUSTOM_DISPLAY &custom, NvU32 index)
        {
            ...
            std::printf("  [%u] %ux%u depth=%u format=%u ratio=%.3f/%.3f hwOnly=%u\n",
                ...
                custom.height,
                custom.depth,
                static_cast<unsigned>(custom.colorFormat),
                custom.xRatio,
                custom.yRatio,
                custom.hwModeSetOnly ? 1 : 0);
            std::printf("       timing: %ux%u %s pclk=%.3f MHz refresh=%.2f Hz\n",
                custom.timing.HVisible,
                custom.timing.VVisible,
                custom.timing.interlaced ? "interlaced" : "progressive",
                static_cast<double>(custom.timing.pclk) / 100.0,
                refresh);
        }

        bool BuildTiming(NvU32 displayId,
            NvU32 width,
            NvU32 height,
            float refresh,
        ...
        bool BuildCustomDisplay(NvU32 displayId,
            NvU32 width,
            NvU32 height,
            float refresh,
            NvU32 depth,
            ...
            NV_CUSTOM_DISPLAY *outCustom)
        {
            ...
            NV_TIMING timing = {};
            if (!BuildTiming(displayId, width, height, refresh, type, interlaced, hasCeaId, ceaId, hasTvFormat, tvFormat, hasPsfId, psfId, &timing))
            {
                return false;
            }
            ...
            if (width == 0)
            {
                width = timing.HVisible;
            }
            if (height == 0)
            {
                height = timing.VVisible;
            }
    ...
    int CmdDisplayCustomList(int argc, char **argv)
    {
        ...
        while (true)
        {
            ...
            if (!any)
            {
                std::printf("Custom displays for 0x%08X:\n", displayId);
            }
            any = true;
        ...
        if (!any)
        {
            std::printf("No custom displays found for 0x%08X.\n", displayId);
        }
        return 0;
    }
    ...
    int CmdDisplayCustomTry(int argc, char **argv)
    {
        ...
        NV_CUSTOM_DISPLAY custom = {};
        if (!BuildCustomDisplay(displayId, width, height, refresh, depth, type, interlaced, hwOnly, hasCeaId, ceaId, hasTvFormat, tvFormat, hasPsfId, psfId, &custom))
        {
            return 1;
        }
    ...
    int CmdDisplayCustom(int argc, char **argv)
    {
        ...
        {
            return CmdDisplayCustomList(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "try") == 0)
        {
            return CmdDisplayCustomTry(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "save") == 0)
        {
            return CmdDisplayCustomSave(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "delete") == 0)
        {
            return CmdDisplayCustomDelete(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "revert") == 0)
        {
            return CmdDisplayCustomRevert(argc - 1, argv + 1);
        }
