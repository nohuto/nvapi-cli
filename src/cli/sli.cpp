...
namespace nvcli
{
    ...
    int CmdSli(int argc, char **argv)
    {
        ...
        return DispatchSubcommand("sli", argc, argv, kSubcommands, sizeof(kSubcommands) / sizeof(kSubcommands[0]), PrintSliUsage);
    }
    ...
    const char *GsyncConnectorName(NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR connector)
    {
        switch (connector)
        {
        case NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR_PRIMARY:
            return "PRIMARY";
        case NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR_SECONDARY:
            return "SECONDARY";
        case NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR_TERTIARY:
            return "TERTIARY";
        case NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR_QUARTERNARY:
            return "QUARTERNARY";
        default:
            return "NONE";
        }
