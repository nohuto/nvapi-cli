...
namespace nvcli
{
    ...
    public:
        NvApiSession()
            : m_status(NvAPI_InitializeEx(NV_DISPLAY_DRIVER))
        {
        }
        ...
        ~NvApiSession()
        {
            ...
            {
                NvAPI_UnloadEx(NV_DISPLAY_DRIVER);
            }
        }
    ...
    public:
        DrsSession()
            : m_handle(NULL)
            , m_status(NvAPI_DRS_CreateSession(&m_handle))
        {
            if (m_status == NVAPI_OK)
            {
                m_status = NvAPI_DRS_LoadSettings(m_handle);
            }
        }

        ~DrsSession()
        {
            if (m_status == NVAPI_OK && m_handle)
            {
                NvAPI_DRS_DestroySession(m_handle);
            }
        }
        ...
        NvDRSSessionHandle handle() const
        {
            return m_handle;
        }
