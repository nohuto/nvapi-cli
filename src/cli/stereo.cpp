...
namespace nvcli
{
    namespace
    {
        ...
        const char *WindowedModeName(NVAPI_STEREO_WINDOWED_MODE mode)
        {
            switch (mode)
            {
            case WINDOWED_MODE_STEREO_OFF:
                return "off";
            case WINDOWED_MODE_STEREO_AUTOMATIC:
                return "automatic";
            case WINDOWED_MODE_STEREO_PERSISTENT:
                return "persistent";
            default:
                return "unknown";
            }
        ...
        const char *ActiveEyeName(NV_STEREO_ACTIVE_EYE eye)
        {
            switch (eye)
            {
            case NVAPI_STEREO_EYE_RIGHT:
                return "right";
            case NVAPI_STEREO_EYE_LEFT:
                return "left";
            case NVAPI_STEREO_EYE_MONO:
                return "mono";
            default:
                return "unknown";
            }
        ...
        const char *SurfaceCreationModeName(NVAPI_STEREO_SURFACECREATEMODE mode)
        {
            switch (mode)
            {
            case NVAPI_STEREO_SURFACECREATEMODE_AUTO:
                return "auto";
            case NVAPI_STEREO_SURFACECREATEMODE_FORCESTEREO:
                return "force-stereo";
            case NVAPI_STEREO_SURFACECREATEMODE_FORCEMONO:
                return "force-mono";
            default:
                return "unknown";
            }
        ...
        const char *StereoSrcLayoutName(STEREO_SRC_LAYOUT layout)
        {
            switch (layout)
            {
            case STEREO_SRC_LAYOUT_LEFT_RIGHT:
                return "left-right";
            case STEREO_SRC_LAYOUT_RIGHT_LEFT:
                return "right-left";
            case STEREO_SRC_LAYOUT_TOP_BOTTOM:
                return "top-bottom";
            case STEREO_SRC_LAYOUT_BOTTOM_TOP:
                return "bottom-top";
            case STEREO_SRC_LAYOUT_MONO_MODE:
                return "mono";
            default:
                return "unknown";
            }
        ...
        const char *ModeEnumCommandName(STEREO_MODE_ENUM_CONTROL command)
        {
            switch (command)
            {
            case STEREO_MODE_ENUM_DISABLE:
                return "disable";
            case STEREO_MODE_ENUM_ENBALE:
                return "enable";
            case STEREO_MODE_ENUM_STATUS:
                return "status";
            default:
                return "unknown";
            }
        ...
        const char *DongleCommandName(STEREO_DONGLE_CONTROL_COMMAND command)
        {
            switch (command)
            {
            case STEREO_DONGLE_COMMAND_POLL:
                return "poll";
            case STEREO_DONGLE_COMMAND_SUPPORTED:
                return "supported";
            case STEREO_DONGLE_COMMAND_SET_TIMING:
                return "set-timing";
            case STEREO_DONGLE_COMMAND_ACTIVATE:
                return "activate";
            case STEREO_DONGLE_COMMAND_DEACTIVATE:
                return "deactivate";
            case STEREO_DONGLE_COMMAND_AEGIS_SURROUND:
                return "aegis-surround";
            case STEREO_DONGLE_COMMAND_GET_ACTIVE_COUNT:
                return "active-count";
            case STEREO_DONGLE_COMMAND_SUPPORTED_DEVINDEPENDENT:
                return "supported-device-independent";
            default:
                return "unknown";
            }
        }
        ...
        void PrintStereoCaps(const NVAPI_STEREO_CAPS &caps)
        {
            std::printf("  windowedOff=%u windowedAuto=%u windowedPersistent=%u\n",
                ...
                caps.supportsWindowedModeAutomatic,
                caps.supportsWindowedModePersistent);
        }
        ...
        int CmdStereoInfo(int argc, char **argv)
        {
            ...
            std::printf("  enabled=%u active=%u clients=%u devices=%d\n",
                ...
                common.status.isStereoActive,
                common.status.numStereoClients,
                common.status.stereoDeviceCount);
            for (NvU32 i = 0; i < NV_STEREO_MAX_HEADS; ++i)
            {
                std::printf("  head[%u] config=0x%08X pending=0x%08X persistent=0x%08X\n",
                    i,
                    common.status.stereoConfig[i],
                    common.status.pendingStereoConfig[i],
                    common.status.persistentStereoConfig[i]);
            }
            std::printf("  emitter init=%u connected=%u activated=%u activeHead=%u activeCount=%u\n",
                common.emitter.isInit,
                common.emitter.isConnected,
                common.emitter.isActivated,
                common.emitter.activatedHead,
                common.emitter.activeCount);
            std::printf("  aero appCount=%u appPid=%u originalRR=%u\n",
                common.aero.appCount,
                common.aero.appPid,
                common.aero.orgRR);
            return 0;
        }
        ...
        int CmdStereoModeEnum(int argc, char **argv)
        {
            ...
            if (std::strcmp(argv[0], "get") == 0)
            {
                ...
                std::printf("Mode enum status: command=%s data=%u\n",
                    ModeEnumCommandName(control.command),
                    control.data);
                return 0;
            }
            if (std::strcmp(argv[0], "set") == 0)
        ...
        int CmdStereoDongle(int argc, char **argv)
        {
            ...
            if (std::strcmp(argv[0], "control") == 0)
            {
                ...
                std::printf("Dongle control ok: command=%s data=%u result=%u\n",
                    DongleCommandName(control.command), control.data, control.result);
                return 0;
            }
        ...
        int CmdStereoActivation(const char *command)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("Stereo activation command failed", status);
            }
            ctx.Cleanup();
        ...
        int CmdStereoSeparation(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo separation failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoConvergence(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo convergence failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoFrustum(int argc, char **argv)
        {
            ...
            if (std::strcmp(argv[0], "get") == 0)
            {
                ...
                if (status == NVAPI_OK)
                {
                    std::printf("Frustum adjust mode: %s\n", FrustumAdjustModeName(mode));
                }
            }
            else if (std::strcmp(argv[0], "set") == 0)
            {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo frustum failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoCapture(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_Capture failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoInitActivation(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_InitActivation failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoTriggerActivation()
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_Trigger_Activation failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoReverseBlit(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_ReverseStereoBlitControl failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoNotify(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_SetNotificationMessage failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoActiveEye(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_SetActiveEye failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoCursor(int argc, char **argv)
        {
            ...
            if (std::strcmp(argv[0], "set") == 0)
            {
                ...
                if (status != NVAPI_OK)
                {
                    PrintNvapiError("NvAPI_Stereo_SetCursorSeparation failed", status);
                }
                ctx.Cleanup();
                return status == NVAPI_OK ? 0 : 1;
            }
        ...
        int CmdStereoSurface(int argc, char **argv)
        {
            ...
            if (std::strcmp(argv[0], "set") == 0)
            {
                ...
                if (status != NVAPI_OK)
                {
                    PrintNvapiError("NvAPI_Stereo_SetSurfaceCreationMode failed", status);
                }
                ctx.Cleanup();
                return status == NVAPI_OK ? 0 : 1;
            }
        ...
        int CmdStereoForceToScreen(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_ForceToScreenDepth failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        int CmdStereoVideoControl(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_SetVideoControl failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoVideoMetadata(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_SetVideoMetadata failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoHandshakeTrigger()
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_HandShake_Trigger_Activation failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoHandshakeMessage(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_HandShake_Message_Control failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        int CmdStereoProfileName(int argc, char **argv)
        {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_Stereo_SetProfileName failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
        ...
        int CmdStereoShader(int argc, char **argv)
        {
            ...
            size_t elementSize = (type == ShaderType::Bool) ? sizeof(BOOL)
                : (type == ShaderType::Int ? sizeof(int) : sizeof(float));
            ...
            if (type == ShaderType::Float)
            {
                ...
                if (isSet)
                {
                    if (!ReadBinaryFile(monoPath, mono.data(), byteSize)
                        || !ReadBinaryFile(leftPath, left.data(), byteSize)
                ...
                if (stage == ShaderStage::Vertex)
                {
                    if (isSet)
                    {
                        status = NvAPI_Stereo_SetVertexShaderConstantF(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                    else
                    {
                        status = NvAPI_Stereo_GetVertexShaderConstantF(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                }
                else
                {
                    if (isSet)
                    {
                        status = NvAPI_Stereo_SetPixelShaderConstantF(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                    else
                    {
                        status = NvAPI_Stereo_GetPixelShaderConstantF(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                }
                if (!isSet && status == NVAPI_OK)
                {
                    if (!WriteBinaryFile(monoPath, mono.data(), byteSize)
                        || !WriteBinaryFile(leftPath, left.data(), byteSize)
            ...
            else if (type == ShaderType::Int)
            {
                ...
                if (stage == ShaderStage::Vertex)
                {
                    if (isSet)
                    {
                        status = NvAPI_Stereo_SetVertexShaderConstantI(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                    else
                    {
                        status = NvAPI_Stereo_GetVertexShaderConstantI(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                }
                else
                {
                    if (isSet)
                    {
                        status = NvAPI_Stereo_SetPixelShaderConstantI(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                    else
                    {
                        status = NvAPI_Stereo_GetPixelShaderConstantI(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                }
                if (!isSet && status == NVAPI_OK)
                {
            ...
            else
            {
                ...
                if (stage == ShaderStage::Vertex)
                {
                    if (isSet)
                    {
                        status = NvAPI_Stereo_SetVertexShaderConstantB(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                    else
                    {
                        status = NvAPI_Stereo_GetVertexShaderConstantB(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                }
                else
                {
                    if (isSet)
                    {
                        status = NvAPI_Stereo_SetPixelShaderConstantB(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                    else
                    {
                        status = NvAPI_Stereo_GetPixelShaderConstantB(ctx.handle, startReg, mono.data(), left.data(), right.data(), count);
                    }
                }
                if (!isSet && status == NVAPI_OK)
                {
            ...
            if (status != NVAPI_OK)
            {
                PrintNvapiError("Stereo shader constants failed", status);
            }
            ctx.Cleanup();
            return status == NVAPI_OK ? 0 : 1;
        }
    ...
    int CmdStereo(int argc, char **argv)
    {
        ...
        {
            return CmdStereoEnable(true);
        }
        if (std::strcmp(argv[0], "disable") == 0)
        {
            return CmdStereoEnable(false);
        }
        if (std::strcmp(argv[0], "is-enabled") == 0)
        {
            return CmdStereoIsEnabled();
        }
        if (std::strcmp(argv[0], "windowed") == 0)
        {
            return CmdStereoWindowed(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "caps") == 0)
        {
            return CmdStereoCaps(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "info") == 0)
        {
            return CmdStereoInfo(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "app-info") == 0)
        {
            return CmdStereoAppInfo(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "mode-enum") == 0)
        {
            return CmdStereoModeEnum(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "accessory") == 0)
        {
            return CmdStereoAccessory();
        }
        if (std::strcmp(argv[0], "dongle") == 0)
        {
            return CmdStereoDongle(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "aegis") == 0)
        {
            return CmdStereoAegis(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "default-profile") == 0)
        {
            return CmdStereoDefaultProfile(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "profile") == 0)
        {
            return CmdStereoProfile(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "driver-mode") == 0)
        {
            return CmdStereoDriverMode(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "activate") == 0 || std::strcmp(argv[0], "deactivate") == 0 || std::strcmp(argv[0], "is-activated") == 0)
        {
            return CmdStereoActivation(argv[0]);
        }
        if (std::strcmp(argv[0], "separation") == 0)
        {
            return CmdStereoSeparation(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "convergence") == 0)
        {
            return CmdStereoConvergence(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "frustum") == 0)
        {
            return CmdStereoFrustum(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "capture") == 0)
        {
            return CmdStereoCapture(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "init-activation") == 0)
        {
            return CmdStereoInitActivation(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "trigger-activation") == 0)
        {
            return CmdStereoTriggerActivation();
        }
        if (std::strcmp(argv[0], "reverse-blit") == 0)
        {
            return CmdStereoReverseBlit(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "notify") == 0)
        {
            return CmdStereoNotify(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "active-eye") == 0)
        {
            return CmdStereoActiveEye(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "eye-separation") == 0)
        {
            return CmdStereoEyeSeparation();
        }
        if (std::strcmp(argv[0], "cursor") == 0)
        {
            return CmdStereoCursor(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "surface") == 0)
        {
            return CmdStereoSurface(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "debug") == 0)
        {
            if (argc > 1 && std::strcmp(argv[1], "last-draw") == 0)
            {
                return CmdStereoDebugLastDraw();
            }
            std::printf("Unknown debug command.\n");
            return 1;
        }
        if (std::strcmp(argv[0], "force-to-screen") == 0)
        {
            return CmdStereoForceToScreen(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "video-control") == 0)
        {
            return CmdStereoVideoControl(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "video-metadata") == 0)
        {
            return CmdStereoVideoMetadata(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "handshake") == 0)
        {
            return CmdStereoHandshake(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "handshake-trigger") == 0)
        {
            return CmdStereoHandshakeTrigger();
        }
        if (std::strcmp(argv[0], "handshake-message") == 0)
        {
            return CmdStereoHandshakeMessage(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "profile-name") == 0)
        {
            return CmdStereoProfileName(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "diag") == 0)
        {
            return CmdStereoDiag();
        }
        if (std::strcmp(argv[0], "shader") == 0)
        {
            return CmdStereoShader(argc - 1, argv + 1);
        }
