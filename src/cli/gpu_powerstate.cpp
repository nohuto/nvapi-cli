...
const char *EventLevelName(NV_EVENT_LEVEL level) {
  switch (level) {
  case UNKNOWN_LEVEL: return "UNKNOWN";
  case NORMAL_LEVEL: return "NORMAL";
...
const char *Gc6ControlOpName(NV_GPU_GC6_CONTROL_OP op) {
  switch (op) {
  case CLEAR_STATS: return "CLEAR_STATS";
  case SET_STATS_ENABLED: return "SET_STATS_ENABLED";
  case SET_STATS_DISABLED: return "SET_STATS_DISABLED";
  case IS_GC6_SUPPORTED: return "IS_GC6_SUPPORTED";
  case IS_GC6_ENABLED: return "IS_GC6_ENABLED";
  default: return "UNKNOWN";
  }
}
...
const char *Gc6ControlStatusName(NV_GPU_GC6_CONTROL_STATUS status) {
  ...
  case CANNOT_ENABLE_STATS: return "CANNOT_ENABLE_STATS";
  case STATS_DISABLED: return "STATS_DISABLED";
...
const char *DeepIdleStateName(NV_DEEP_IDLE_STATE state) {
  switch (state) {
  case NV_DEEP_IDLE_ENABLE: return "ENABLED";
  case NV_DEEP_IDLE_DISABLE: return "DISABLED";
  case NV_DEEP_IDLE_NOT_SUPPORTED: return "NOT_SUPPORTED";
  default: return "UNKNOWN";
  }
}
...
const char *DeepIdleStatsModeName(NV_DEEP_IDLE_STATISTICS_MODE mode) {
  switch (mode) {
  case NV_DEEP_IDLE_STATISTICS_MODE_NH: return "NH";
  case NV_DEEP_IDLE_STATISTICS_MODE_VE: return "VE";
  case NV_DEEP_IDLE_STATISTICS_MODE_SSC: return "SSC";
  case NV_DEEP_IDLE_STATISTICS_MODE_FO: return "FO";
  default: return "UNKNOWN";
  }
}
...
const char *PstateFallbackName(NV_GPU_PERF_PSTATE_FALLBACK fallback) {
  switch (fallback) {
  case NVAPI_GPU_PERF_PSTATE_FALLBACK_RETURN_ERROR: return "ERROR";
  case NVAPI_GPU_PERF_PSTATE_FALLBACK_HIGHER_PERF: return "HIGHER";
  case NVAPI_GPU_PERF_PSTATE_FALLBACK_LOWER_PERF: return "LOWER";
  default: return "UNKNOWN";
  }
}
...
int CmdGpuGc6Control(int argc, char **argv) {
  ...
  for (size_t i = 0; i < handles.size(); ++i) {
    ...
    std::printf("  GC6 op=%s status=%s (%u)\n", Gc6ControlOpName(op), Gc6ControlStatusName(control.controlStatus),
...
int CmdGpuGc6(int argc, char **argv) {
  ...
  if (std::strcmp(argv[0], "control") == 0) { return CmdGpuGc6Control(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "force-exit") == 0) { return CmdGpuGc6ForceExit(argc - 1, argv + 1); }
...
int CmdGpuDeepIdle(int argc, char **argv) {
  ...
  if (std::strcmp(argv[0], "state") == 0) { return CmdGpuDeepIdleState(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "set") == 0) { return CmdGpuDeepIdleSet(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "stats-mode") == 0) { return CmdGpuDeepIdleStatsMode(argc - 1, argv + 1); }
  if (std::strcmp(argv[0], "stats") == 0) { return CmdGpuDeepIdleStats(argc - 1, argv + 1); }
