...
namespace nvcli
{
    int CmdVideoColorGet(NvDisplayHandle handle, bool useDefault)
    {
        ...
        if (status != NVAPI_OK)
        {
            PrintNvapiError(useDefault ? "NvAPI_Video_ColorControl_GetDefault failed" : "NvAPI_Video_ColorControl_Get failed", status);
            return 1;
        }

        std::printf("Video color control (%s):\n", useDefault ? "default" : "current");
        std::printf("  controlSetting=%u feature=%u caps=%u\n",
            info.colorControlSettings,
            info.colorFeature,
            info.colorCaps);
        std::printf("  brightness=%u (min=%u max=%u)\n",
            info.brightnessRangeValue.value,
            info.brightnessRangeValue.min,
            info.brightnessRangeValue.max);
        std::printf("  contrast=%u (min=%u max=%u)\n",
            info.contrastRangeValue.value,
            info.contrastRangeValue.min,
            info.contrastRangeValue.max);
        std::printf("  hue=%u (min=%u max=%u)\n",
            info.hueRangeValue.value,
            info.hueRangeValue.min,
            info.hueRangeValue.max);
        std::printf("  saturation=%u (min=%u max=%u)\n",
            info.saturationRangeValue.value,
            info.saturationRangeValue.min,
            info.saturationRangeValue.max);
        std::printf("  colorTemp=%u (min=%u max=%u) feature=%u caps=%u\n",
            info.colorTempRangeValue.value,
            info.colorTempRangeValue.min,
            info.colorTempRangeValue.max,
            info.colorTempFeature,
            info.colorTempCaps);
        std::printf("  gammaFeature=%u caps=%u\n",
            info.gammaFeature,
            info.gammaCaps);
        std::printf("  yGamma=%u (min=%u max=%u)\n",
            info.yGammaRangeValue.value,
            info.yGammaRangeValue.min,
            info.yGammaRangeValue.max);
        std::printf("  rgbGammaR=%u (min=%u max=%u)\n",
            info.rgbGammaRRangeValue.value,
            info.rgbGammaRRangeValue.min,
            info.rgbGammaRRangeValue.max);
        std::printf("  rgbGammaG=%u (min=%u max=%u)\n",
            info.rgbGammaGRangeValue.value,
            info.rgbGammaGRangeValue.min,
            info.rgbGammaGRangeValue.max);
        std::printf("  rgbGammaB=%u (min=%u max=%u)\n",
            info.rgbGammaBRangeValue.value,
            info.rgbGammaBRangeValue.min,
    ...
    int CmdVideoColorSet(int argc, char **argv)
    {
        ...
        if (!(hasBrightness || hasContrast || hasHue || hasSaturation || hasTemp ||
              hasYGamma || hasRGamma || hasGGamma || hasBGamma || hasControlSetting))
        {
    ...
    int CmdVideo(int argc, char **argv)
    {
        ...
        {
            if (argc >= 2 && std::strcmp(argv[1], "get") == 0)
            {
                return CmdVideoColorGet(argc - 2, argv + 2, false);
            }
            if (argc >= 2 && std::strcmp(argv[1], "default") == 0)
            {
                return CmdVideoColorGet(argc - 2, argv + 2, true);
            }
            if (argc >= 2 && std::strcmp(argv[1], "set") == 0)
            {
                return CmdVideoColorSet(argc - 2, argv + 2);
            }
    ...
    const char *HdmiFrlRateName(NV_HDMI_FRL_RATE rate)
    {
        switch (rate)
        {
        case NV_HDMI_FRL_RATE_NONE:
            return "NONE";
        case NV_HDMI_FRL_RATE_3LANES_3GBPS:
            return "3LANES_3GBPS";
        case NV_HDMI_FRL_RATE_3LANES_6GBPS:
            return "3LANES_6GBPS";
        case NV_HDMI_FRL_RATE_4LANES_6GBPS:
            return "4LANES_6GBPS";
        case NV_HDMI_FRL_RATE_4LANES_8GBPS:
            return "4LANES_8GBPS";
        case NV_HDMI_FRL_RATE_4LANES_10GBPS:
            return "4LANES_10GBPS";
        case NV_HDMI_FRL_RATE_4LANES_12GBPS:
            return "4LANES_12GBPS";
        default:
            return "UNKNOWN";
        }
}
