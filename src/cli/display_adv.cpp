...
namespace nvcli
{
    namespace
    {
        ...
        const char *MonitorCapsTypeName(NV_MONITOR_CAPS_TYPE type)
        {
            switch (type)
            {
            case NV_MONITOR_CAPS_TYPE_HDMI_VSDB:
                return "HDMI_VSDB";
            case NV_MONITOR_CAPS_TYPE_HDMI_VCDB:
                return "HDMI_VCDB";
            case NV_MONITOR_CAPS_TYPE_GENERIC:
                return "GENERIC";
            default:
                return "UNKNOWN";
            }
        ...
        void PrintScalingCapsV1(const NV_GET_SCALING_CAPS_V1 &caps)
        {
            ...
            std::printf("  gpuScaling=%u gpuAspect=%u monitorScaling=%u centerScaling=%u\n",
                ...
                caps.isGPUFixedAspectRatioScalingAvailable ? 1 : 0,
                caps.isMonitorScalingAvailable ? 1 : 0,
        ...
        void PrintScalingCapsV2(const NV_GET_SCALING_CAPS_V2 &caps)
        {
            ...
            std::printf("  gpuAspectClosest=%u gpuCenterClosest=%u gpuInteger=%u\n",
                caps.isGPUScalingToAspectScanoutToClosest ? 1 : 0,
                caps.isGPUScanoutToClosest ? 1 : 0,
                caps.isGpuRepeatToAspectScanoutToNative ? 1 : 0);
            std::printf("  default=%s current=%s\n",
                ScalingName(caps.defaultScaling),
                ScalingName(caps.currentScaling));
        }
        ...
        bool ApplyForcedScalingCapsV1(NV_GET_SCALING_CAPS_V1 &caps, NV_SCALING preferred)
        {
            ...
            {
            case NV_SCALING_GPU_SCALING_TO_CLOSEST:
                caps.isMonitorScalingAvailable = 1;
                return true;
            case NV_SCALING_GPU_SCALING_TO_NATIVE:
                caps.isGPUScalingAvailable = 1;
                return true;
            case NV_SCALING_GPU_SCANOUT_TO_NATIVE:
                caps.isCenterScalingAvailable = 1;
                return true;
            case NV_SCALING_GPU_SCALING_TO_ASPECT_SCANOUT_TO_NATIVE:
                caps.isGPUFixedAspectRatioScalingAvailable = 1;
                return true;
            case NV_SCALING_GPU_SCALING_TO_ASPECT_SCANOUT_TO_CLOSEST:
                caps.isMonitorScalingAvailable = 1;
                return true;
        ...
        bool ApplyForcedScalingCapsV2(NV_GET_SCALING_CAPS_V2 &caps, NV_SCALING preferred)
        {
            ...
            case NV_SCALING_GPU_SCALING_TO_ASPECT_SCANOUT_TO_NATIVE:
                ...
                return true;
            case NV_SCALING_GPU_SCALING_TO_ASPECT_SCANOUT_TO_CLOSEST:
                caps.isGPUScalingToAspectScanoutToClosest = 1;
                return true;
            case NV_SCALING_GPU_SCANOUT_TO_CLOSEST:
                caps.isGPUScanoutToClosest = 1;
                return true;
            case NV_SCALING_GPU_INTEGER_ASPECT_SCALING:
                caps.isGpuRepeatToAspectScanoutToNative = 1;
                return true;
            default:
                return false;
            }
    ...
    int CmdDisplayMonitorCaps(int argc, char **argv)
    {
        ...
        for (size_t i = 0; i < types.size(); ++i)
        {
            ...
            std::printf("Monitor caps type=%s connector=%s valid=%u\n",
                ...
                ConnectorTypeName(static_cast<NV_MONITOR_CONN_TYPE>(caps.connectorType)),
                caps.bIsValidInfo ? 1 : 0);

            if (!caps.bIsValidInfo)
            {
                continue;
            }
            ...
            if (caps.infoType == NV_MONITOR_CAPS_TYPE_GENERIC)
            {
                ...
                std::printf("  vrr=%u ulmb=%u trueGsync=%u rla=%u\n",
                    info.supportVRR ? 1 : 0,
                    info.supportULMB ? 1 : 0,
                    info.isTrueGsync ? 1 : 0,
                    info.isRLACapable ? 1 : 0);
            }
            else if (caps.infoType == NV_MONITOR_CAPS_TYPE_HDMI_VSDB)
            {
                ...
                std::printf("  physAddr=%u.%u.%u.%u maxTmdsClock=%u\n",
                    info.sourcePhysicalAddressA,
                    info.sourcePhysicalAddressB,
                    info.sourcePhysicalAddressC,
                    info.sourcePhysicalAddressD,
                    info.maxTmdsClock);
                std::printf("  deepColor: ycbcr444=%u 30=%u 36=%u 48=%u ai=%u dualDvi=%u\n",
                    info.supportDeepColorYCbCr444 ? 1 : 0,
                    info.supportDeepColor30bits ? 1 : 0,
                    info.supportDeepColor36bits ? 1 : 0,
                    info.supportDeepColor48bits ? 1 : 0,
                    info.supportAI ? 1 : 0,
                    info.supportDualDviOperation ? 1 : 0);
                std::printf("  cnc: graphics=%u photo=%u cinema=%u game=%u\n",
                    info.cnc0SupportGraphicsTextContent ? 1 : 0,
                    info.cnc1SupportPhotoContent ? 1 : 0,
                    info.cnc2SupportCinemaContent ? 1 : 0,
                    info.cnc3SupportGameContent ? 1 : 0);
                std::printf("  latency: has=%u interlaced=%u video=%u audio=%u\n",
                    info.hasLatencyField ? 1 : 0,
                    info.hasInterlacedLatencyField ? 1 : 0,
                    info.videoLatency,
                    info.audioLatency);
                std::printf("  latencyInterlaced: video=%u audio=%u\n",
                    info.interlacedVideoLatency,
                    info.interlacedAudioLatency);
                std::printf("  vic: has=%u len=%u 3d: has=%u len=%u\n",
                    info.hasVicEntries ? 1 : 0,
                    info.hdmiVicLength,
                    info.has3dEntries ? 1 : 0,
                    info.hdmi3dLength);
            ...
            else if (caps.infoType == NV_MONITOR_CAPS_TYPE_HDMI_VCDB)
            {
                ...
                std::printf("  quantYcc=%u quantRgb=%u scanPref=%u scanIT=%u scanCE=%u\n",
                    ...
                    info.quantizationRangeRgb,
                    info.scanInfoPreferredVideoFormat,
                    info.scanInfoITVideoFormats,
    ...
    int CmdDisplayMonitorColorCaps(int argc, char **argv)
    {
        ...
        {
            caps[i].version = NV_MONITOR_COLOR_CAPS_VER;
        }
        ...
        for (NvU32 i = 0; i < count; ++i)
        {
            std::printf("  [%u] format=%s bpc=%s\n",
                i,
                DpColorFormatName(caps[i].colorFormat),
    ...
    int CmdDisplayScalingOverrideSet(int argc, char **argv)
    {
        ...
        if (usedV1)
        {
            ...
            {
                bool emptyCaps = !capsV1.isGPUScalingAvailable &&
                                 !capsV1.isGPUFixedAspectRatioScalingAvailable &&
                                 !capsV1.isMonitorScalingAvailable &&
                                 !capsV1.isCenterScalingAvailable;
                if (emptyCaps)
                {
        ...
        if (capsFromOverride)
        {
            bool emptyCaps = !capsV2.isGPUScalingAvailable &&
                             !capsV2.isGPUFixedAspectRatioScalingAvailable &&
                             !capsV2.isMonitorScalingAvailable &&
                             !capsV2.isCenterScalingAvailable &&
                             !capsV2.isGPUScalingToAspectScanoutToClosest &&
                             !capsV2.isGPUScanoutToClosest &&
    ...
    int CmdDisplayScalingOverride(int argc, char **argv)
    {
        ...
        {
            return CmdDisplayScalingOverrideGet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "set") == 0)
        {
            return CmdDisplayScalingOverrideSet(argc - 1, argv + 1);
        }
    ...
    int CmdDisplayViewport(int argc, char **argv)
    {
        ...
        if (std::strcmp(argv[0], "get") == 0)
        {
            return CmdDisplayViewportGet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "set") == 0)
        {
            return CmdDisplayViewportSet(argc - 1, argv + 1);
        }
    ...
    int CmdDisplayFeatureSet(int argc, char **argv)
    {
        ...
        if (hasPanScan)
        {
            config.isPanAndScanEnabled = panScan ? 1 : 0;
        }
        if (hasGdiPrimary)
        {
            config.modulePresentSyncGDIPrimaryTarget = gdiPrimary ? 1 : 0;
        }
    ...
    int CmdDisplayFeature(int argc, char **argv)
    {
        ...
        if (std::strcmp(argv[0], "get") == 0)
        {
            return CmdDisplayFeatureGet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "set") == 0)
        {
            return CmdDisplayFeatureSet(argc - 1, argv + 1);
        }
    ...
    int CmdDisplayWideColorGet(int argc, char **argv)
    {
        ...
        std::printf("Wide color range: range=%s enable=%u\n",
            WideColorRangeName(setting.colorRange),
            setting.enable ? 1 : 0);
        return 0;
    }
    ...
    int CmdDisplayWideColor(int argc, char **argv)
    {
        ...
        if (std::strcmp(argv[0], "get") == 0)
        {
            return CmdDisplayWideColorGet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "set") == 0)
        {
            return CmdDisplayWideColorSet(argc - 1, argv + 1);
        }
    ...
    int CmdDisplayBpcGet(int argc, char **argv)
    {
        ...
        std::printf("BPC config: bpc=%u ditherOff=%u forceLink=%u forceRGDiv=%u\n",
            config.bpc,
            config.ditherOff ? 1 : 0,
            config.forceAtCurLinkConfig ? 1 : 0,
            config.forceRGDivMode ? 1 : 0);
        return 0;
    }
    ...
    int CmdDisplayBpcSet(int argc, char **argv)
    {
        ...
        for (int i = 0; i < argc; ++i)
        {
            ...
            {
                if (i + 1 >= argc || !ParseUint(argv[i + 1], &bpc))
                {
        ...
        if (hasDither)
        {
            config.ditherOff = dither ? 0 : 1;
        }
        if (hasForceLink)
        {
            config.forceAtCurLinkConfig = forceLink ? 1 : 0;
        }
        if (hasForceRgDiv)
        {
            config.forceRGDivMode = forceRgDiv ? 1 : 0;
        }

        config.cmd = NV_BPC_CONFIG_CMD_SET;

        std::printf("BPC set: bpc=%u ditherOff=%u forceLink=%u forceRGDiv=%u\n",
            config.bpc,
            config.ditherOff ? 1 : 0,
            config.forceAtCurLinkConfig ? 1 : 0,
            config.forceRGDivMode ? 1 : 0);
    ...
    int CmdDisplayBpc(int argc, char **argv)
    {
        ...
        if (std::strcmp(argv[0], "get") == 0)
        {
            return CmdDisplayBpcGet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "set") == 0)
        {
            return CmdDisplayBpcSet(argc - 1, argv + 1);
        }
    ...
    int CmdDisplayBlankingSet(int argc, char **argv)
    {
        ...
        if (hasPersist)
        {
            info.persistBlankingAcrossHotPlugUnplug = persist ? 1 : 0;
        }
    ...
    int CmdDisplayBlanking(int argc, char **argv)
    {
        ...
        if (std::strcmp(argv[0], "get") == 0)
        {
            return CmdDisplayBlankingGet(argc - 1, argv + 1);
        }
        if (std::strcmp(argv[0], "set") == 0)
        {
            return CmdDisplayBlankingSet(argc - 1, argv + 1);
        }
