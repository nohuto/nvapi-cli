...
void PrintGsyncUsage() { PrintUsageGroup("gsync"); }
...
int CmdGsyncSyncDispatch(int argc, char **argv) {
  ...
  return DispatchSubcommand("gsync sync", argc, argv, kSubcommands, sizeof(kSubcommands) / sizeof(kSubcommands[0]),
                            PrintGsyncUsage);
}

const char *GsyncPolarityName(NVAPI_GSYNC_POLARITY polarity) {
  switch (polarity) {
  case NVAPI_GSYNC_POLARITY_RISING_EDGE: return "RISING";
  case NVAPI_GSYNC_POLARITY_FALLING_EDGE: return "FALLING";
  case NVAPI_GSYNC_POLARITY_BOTH_EDGES: return "BOTH";
...
const char *GsyncVideoModeName(NVAPI_GSYNC_VIDEO_MODE mode) {
  switch (mode) {
  case NVAPI_GSYNC_VIDEO_MODE_NONE: return "NONE";
  case NVAPI_GSYNC_VIDEO_MODE_TTL: return "TTL";
  case NVAPI_GSYNC_VIDEO_MODE_NTSCPALSECAM: return "NTSCPALSECAM";
  case NVAPI_GSYNC_VIDEO_MODE_HDTV: return "HDTV";
  case NVAPI_GSYNC_VIDEO_MODE_COMPOSITE: return "COMPOSITE";
  default: return "UNKNOWN";
  }
}
...
const char *GsyncSyncSourceName(NVAPI_GSYNC_SYNC_SOURCE source) {
  switch (source) {
  case NVAPI_GSYNC_SYNC_SOURCE_VSYNC: return "VSYNC";
  case NVAPI_GSYNC_SYNC_SOURCE_HOUSESYNC: return "HOUSESYNC";
  default: return "UNKNOWN";
  }
}
...
const char *GsyncRj45IoName(NVAPI_GSYNC_RJ45_IO value) {
  switch (value) {
  case NVAPI_GSYNC_RJ45_OUTPUT: return "OUTPUT";
  case NVAPI_GSYNC_RJ45_INPUT: return "INPUT";
  case NVAPI_GSYNC_RJ45_UNUSED: return "UNUSED";
  default: return "UNKNOWN";
  }
}
...
int CmdGsyncList() {
  ...
  for (NvU32 i = 0; i < count; ++i) { std::printf("  [%u] handle=0x%p\n", i, handles[i]); }
  return 0;
}
...
int CmdGsyncCaps(int argc, char **argv) {
  ...
  if (!GetGsyncHandleByIndex(index, &handle)) { return 1; }
...
int CmdGsyncTopology(int argc, char **argv) {
  ...
  for (NvU32 i = 0; i < gpuCount; ++i) { gpus[i].version = NV_GSYNC_GPU_VER; }
  for (NvU32 i = 0; i < displayCount; ++i) { displays[i].version = NV_GSYNC_DISPLAY_VER; }
  ...
  for (NvU32 i = 0; i < gpuCount; ++i) {
    ...
    std::printf("  GPU[%u] handle=0x%p name=%s connector=%s proxy=0x%p synced=%u\n", i, gpus[i].hPhysicalGpu, name,
                GsyncConnectorName(gpus[i].connector), gpus[i].hProxyPhysicalGpu, gpus[i].isSynced ? 1u : 0u);
  }

  for (NvU32 i = 0; i < displayCount; ++i) {
    std::printf("  display[%u] id=0x%08X state=%s masterable=%u\n", i, displays[i].displayId,
                GsyncDisplaySyncStateName(displays[i].syncState), displays[i].isMasterable ? 1u : 0u);
  }
...
int CmdGsyncSyncSet(int argc, char **argv) {
  ...
  std::printf("G-Sync sync state update: displayId=0x%08X state=%s flags=0x%08X\n", displayId,
              GsyncDisplaySyncStateName(state), flags);
...
int CmdGsyncStatus(int argc, char **argv) {
  ...
  if (hasGpuIndex) {
    ...
    if (!CollectGpus(true, gpuIndex, handles, indices)) { return 1; }
    if (handles.empty()) {
...
int CmdGsyncControlGet(int argc, char **argv) {
  ...
  std::printf("  polarity=%s videoMode=%s interval=%u\n", GsyncPolarityName(params.polarity),
              GsyncVideoModeName(params.vmode), params.interval);
  std::printf("  source=%s interlace=%u syncSourceOutput=%u\n", GsyncSyncSourceName(params.source),
              params.interlaceMode ? 1u : 0u, params.syncSourceIsOutput ? 1u : 0u);
...
int CmdGsyncControlSet(int argc, char **argv) {
  ...
  if (hasPolarity) { params.polarity = polarity; }
  if (hasVideoMode) { params.vmode = videoMode; }
  if (hasInterval) { params.interval = interval; }
  if (hasSource) { params.source = source; }
  if (hasInterlace) { params.interlaceMode = interlace ? 1u : 0u; }
  if (hasSyncSourceOutput) { params.syncSourceIsOutput = syncSourceOutput ? 1u : 0u; }
...
int CmdGsyncControl(int argc, char **argv) {
  ...
  return DispatchSubcommand("gsync control", argc, argv, kSubcommands, sizeof(kSubcommands) / sizeof(kSubcommands[0]),
                            PrintGsyncUsage);
}
...
int CmdGsync(int argc, char **argv) {
  ...
  static const SubcommandEntry kSubcommands[] = {
      {"list", CmdGsyncListAdapter},  {"caps", CmdGsyncCaps},     {"topo", CmdGsyncTopology},
      {"sync", CmdGsyncSyncDispatch}, {"status", CmdGsyncStatus}, {"control", CmdGsyncControl},
  };

  return DispatchSubcommand("gsync", argc, argv, kSubcommands, sizeof(kSubcommands) / sizeof(kSubcommands[0]),
                            PrintGsyncUsage);
}
} // namespace nvcli
