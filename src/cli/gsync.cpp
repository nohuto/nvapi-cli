...
namespace nvcli
{
    ...
    int CmdGsyncCaps(int argc, char **argv)
    {
        ...
        std::printf("G-Sync[%u] caps: boardId=%u rev=%u extRev=%u\n",
            index,
            caps.boardId,
            caps.revision,
            caps.extendedRevision);
        std::printf("  maxSyncSkew=%u syncSkewResolution=%u\n", caps.maxSyncSkew, caps.syncSkewResolution);
        std::printf("  maxStartDelay=%u startDelayResolution=%u\n", caps.maxStartDelay, caps.startDelayResolution);
        return 0;
    }
    ...
    int CmdGsyncTopology(int argc, char **argv)
    {
        ...
        status = NvAPI_GSync_GetTopology(handle, &gpuCount, gpus.empty() ? NULL : gpus.data(),
            &displayCount, displays.empty() ? NULL : displays.data());
        if (status != NVAPI_OK)
        {
            PrintNvapiError("NvAPI_GSync_GetTopology failed", status);
            return 1;
        ...
        std::printf("G-Sync[%u] topology:\n", index);
        std::printf("  GPUs: %u\n", gpuCount);
        for (NvU32 i = 0; i < gpuCount; ++i)
        {
            NvAPI_ShortString name = {0};
            NvAPI_GPU_GetFullName(gpus[i].hPhysicalGpu, name);
            std::printf("    [%u] %s connector=%s synced=%u proxy=0x%p\n",
                i,
                name,
        ...
        std::printf("  Displays: %u\n", displayCount);
        for (NvU32 i = 0; i < displayCount; ++i)
        {
            std::printf("    [%u] displayId=0x%08X masterable=%u syncState=%s\n",
                i,
                displays[i].displayId,
                displays[i].isMasterable ? 1 : 0,
    ...
    int CmdGsyncSyncGet(int argc, char **argv)
    {
        ...
        std::printf("G-Sync[%u] display sync states:\n", index);
        for (NvU32 i = 0; i < displayCount; ++i)
        {
            std::printf("  displayId=0x%08X state=%s masterable=%u\n",
                displays[i].displayId,
                GsyncDisplaySyncStateName(displays[i].syncState),
                displays[i].isMasterable ? 1 : 0);
        }
    ...
    int CmdGsyncStatus(int argc, char **argv)
    {
        ...
        NvPhysicalGpuHandle gpuHandle = NULL;
        if (hasGpuIndex)
        {
        ...
        {
            NvU32 gpuCount = 0;
            NvAPI_Status status = NvAPI_GSync_GetTopology(handle, &gpuCount, NULL, NULL, NULL);
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_GSync_GetTopology failed", status);
                return 1;
            ...
            if (gpuCount == 0)
            {
                std::printf("No GPUs in G-Sync topology.\n");
                return 1;
            }
            std::vector<NV_GSYNC_GPU> gpus(gpuCount);
            for (NvU32 i = 0; i < gpuCount; ++i)
            ...
            status = NvAPI_GSync_GetTopology(handle, &gpuCount, gpus.data(), NULL, NULL);
            if (status != NVAPI_OK)
            {
                PrintNvapiError("NvAPI_GSync_GetTopology failed", status);
                return 1;
            }
            gpuHandle = gpus[0].hPhysicalGpu;
        }

        NV_GSYNC_STATUS statusInfo = {};
        statusInfo.version = NV_GSYNC_STATUS_VER;
        NvAPI_Status status = NvAPI_GSync_GetSyncStatus(handle, gpuHandle, &statusInfo);
        if (status != NVAPI_OK)
        {
        ...
        status = NvAPI_GSync_GetStatusParameters(handle, &params);
        if (status != NVAPI_OK)
        {
        ...
        std::printf("G-Sync[%u] status:\n", index);
        std::printf("  synced=%u stereoSynced=%u signalAvailable=%u\n",
            statusInfo.bIsSynced ? 1 : 0,
            statusInfo.bIsStereoSynced ? 1 : 0,
            statusInfo.bIsSyncSignalAvailable ? 1 : 0);
        std::printf("  refreshRate=%u houseSync=%u houseSyncIncoming=%u internalSlave=%u\n",
            params.refreshRate,
            params.bHouseSync ? 1 : 0,
            params.houseSyncIncoming,
            params.bInternalSlave ? 1 : 0);
        return 0;
    }
    ...
    int CmdGsyncControlGet(int argc, char **argv)
    {
        ...
        std::printf("G-Sync[%u] control parameters:\n", index);
        std::printf("  polarity=%s vmode=%s interval=%u source=%s interlace=%u\n",
            GsyncPolarityName(params.polarity),
            GsyncVideoModeName(params.vmode),
            params.interval,
            GsyncSyncSourceName(params.source),
            params.interlaceMode ? 1 : 0);
        std::printf("  syncSourceIsOutput=%u skew=%d startDelay=%d\n",
            params.syncSourceIsOutput ? 1 : 0,
            params.skew,
            params.startDelay);
        std::printf("  syncSkew lines=%u pixels=%u maxLines=%u minPixels=%u\n",
            params.syncSkew.numLines,
            params.syncSkew.numPixels,
            params.syncSkew.maxLines,
            params.syncSkew.minPixels);
        std::printf("  startupDelay lines=%u pixels=%u maxLines=%u minPixels=%u\n",
            params.startupDelay.numLines,
            params.startupDelay.numPixels,
            params.startupDelay.maxLines,
            params.startupDelay.minPixels);
        return 0;
    }
    ...
    int CmdGsyncControlSet(int argc, char **argv)
    {
        ...
        NVAPI_GSYNC_VIDEO_MODE vmode = NVAPI_GSYNC_VIDEO_MODE_NONE;
        ...
        for (int i = 0; i < argc; ++i)
        {
            ...
            {
                if (i + 1 >= argc || !ParseGsyncVideoMode(argv[i + 1], &vmode))
                {
        ...
        if (!hasPolarity && !hasVmode && !hasInterval && !hasSource && !hasInterlace && !hasSyncSourceOutput)
        {
            std::printf("No control changes specified.\n");
            return 1;
        }
        ...
        if (hasInterlace)
        {
            params.interlaceMode = interlace ? 1 : 0;
        }
        if (hasSyncSourceOutput)
        {
            params.syncSourceIsOutput = syncSourceOutput ? 1 : 0;
        }

        std::printf("G-Sync control set: polarity=%s vmode=%s interval=%u source=%s interlace=%u syncSourceOutput=%u\n",
            GsyncPolarityName(params.polarity),
            GsyncVideoModeName(params.vmode),
            params.interval,
            GsyncSyncSourceName(params.source),
