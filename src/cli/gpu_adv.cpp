...
namespace nvcli
{
    ...
    static std::string AppendGpuIndexToPath(const char *path, NvU32 index)
    {
        ...
        if (!hasExt)
        {
            return value + "_gpu" + std::to_string(index);
        }
    ...
    {
        if (!args.hasDomain && !args.hasClassType)
        {
            return;
        }
        ...
        if (args.hasDomain)
        {
            params->clkDomainIdx = args.domain;
        }
        if (args.hasClassType)
        {
            params->classType = static_cast<NvU8>(args.classType);
        }
    }
    ...
    {
        return NvAPI_GPU_ClockCounterMeasureAvgFreq(gpu, reinterpret_cast<NV_GPU_CLOCK_COUNTER_MEASURE_AVG_FREQ_PARAMS *>(data));
    }
    ...
    {
        return NvAPI_GPU_ClockClkFreqControllerGetStatus(gpu, reinterpret_cast<NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_STATUS *>(data));
    }

    static NvAPI_Status CallClockClkFreqControllersGetControl(NvPhysicalGpuHandle gpu, void *data)
    {
        return NvAPI_GPU_ClockClkFreqControllersGetControl(gpu, reinterpret_cast<NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_CONTROL *>(data));
    }

    static NvAPI_Status CallClockClkFreqControllersSetControl(NvPhysicalGpuHandle gpu, void *data)
    {
        return NvAPI_GPU_ClockClkFreqControllersSetControl(gpu, reinterpret_cast<NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_CONTROL *>(data));
    }
    ...
    {
        return NvAPI_GPU_ClockClkVoltControllerGetStatus(gpu, reinterpret_cast<NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_STATUS *>(data));
    }

    static NvAPI_Status CallClockClkVoltControllersGetControl(NvPhysicalGpuHandle gpu, void *data)
    {
        return NvAPI_GPU_ClockClkVoltControllersGetControl(gpu, reinterpret_cast<NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_CONTROL *>(data));
    }

    static NvAPI_Status CallClockClkVoltControllersSetControl(NvPhysicalGpuHandle gpu, void *data)
    {
        return NvAPI_GPU_ClockClkVoltControllersSetControl(gpu, reinterpret_cast<NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_CONTROL *>(data));
    }
    ...
    {
        return NvAPI_GPU_ClockClientClkVfPointsGetStatus(gpu, reinterpret_cast<PNV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_STATUS>(data));
    }

    static NvAPI_Status CallClockClientClkVfPointsGetControl(NvPhysicalGpuHandle gpu, void *data)
    {
        return NvAPI_GPU_ClockClientClkVfPointsGetControl(gpu, reinterpret_cast<PNV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_CONTROL>(data));
    }

    static NvAPI_Status CallClockClientClkVfPointsSetControl(NvPhysicalGpuHandle gpu, void *data)
    {
        return NvAPI_GPU_ClockClientClkVfPointsSetControl(gpu, reinterpret_cast<PNV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_CONTROL>(data));
    }

    static NvAPI_Status CallClockPmumonClkDomainsGetSamples(NvPhysicalGpuHandle gpu, void *data)
    {
        return NvAPI_GPU_ClockPmumonClkDomainsGetSamples(gpu, reinterpret_cast<NV_GPU_CLOCK_PMUMON_CLK_DOMAINS_GET_SAMPLES *>(data));
    }
    ...
    {
        return NvAPI_GPU_FanPmumonFanCoolersGetSamples(gpu, reinterpret_cast<NV_GPU_FAN_PMUMON_FAN_COOLERS_GET_SAMPLES *>(data));
    }
    ...
    {
        return NvAPI_GPU_ThermHwFsSlowdownAmountGet(gpu, reinterpret_cast<NV_GPU_THERMAL_HWFS_SLOWDOWN_AMOUNT_GET_PARAMS *>(data));
    }
    ...
    {
        return NvAPI_GPU_ThermalPmumonThermChannelsGetSamples(gpu, reinterpret_cast<NV_GPU_THERMAL_PMUMON_THERM_CHANNELS_GET_SAMPLES *>(data));
    }
    ...
    {
        return NvAPI_GPU_PerfPmumonPerfPoliciesGetSamples(gpu, reinterpret_cast<NV_GPU_PERF_PMUMON_PERF_POLICIES_GET_SAMPLES *>(data));
    }

    static const ApiSpec kClockApiSpecs[] =
    {
        {"counter-avgfreq", "Clock counter average frequency", sizeof(NV_GPU_CLOCK_COUNTER_MEASURE_AVG_FREQ_PARAMS), NV_GPU_CLOCK_COUNTER_MEASURE_AVG_FREQ_PARAMS_VER, false, CallClockCounterMeasureAvgFreq, PrepareClockCounterAvgFreq},
        {"clk-domains-info", "CLK_DOMAIN info", sizeof(NV_GPU_CLOCK_CLK_DOMAINS_INFO), NV_GPU_CLOCK_CLK_DOMAINS_INFO_VER, false, CallClockClkDomainsGetInfo, nullptr},
        {"clk-domains-control", "CLK_DOMAIN control", sizeof(NV_GPU_CLOCK_CLK_DOMAINS_CONTROL), NV_GPU_CLOCK_CLK_DOMAINS_CONTROL_VER, false, CallClockClkDomainsGetControl, nullptr},
        {"clk-domains-set", "CLK_DOMAIN set control", sizeof(NV_GPU_CLOCK_CLK_DOMAINS_CONTROL), NV_GPU_CLOCK_CLK_DOMAINS_CONTROL_VER, true, CallClockClkDomainsSetControl, nullptr},
        {"clk-domains-freq-info", "CLK_DOMAIN frequency info", sizeof(NV_GPU_CLOCK_CLK_DOMAINS_FREQ_INFO), NV_GPU_CLOCK_CLK_DOMAINS_FREQ_INFO_VER, false, CallClockClkDomainsGetFreqInfo, PrepareClockDomainsFreqInfo},
        {"clk-domain-freqs", "CLK_DOMAIN frequency enumeration", sizeof(NV_GPU_CLOCK_CLK_DOMAIN_FREQS_ENUM), NV_GPU_CLOCK_CLK_DOMAIN_FREQS_ENUM_VER, false, CallClockClkDomainFreqsEnum, PrepareClockDomainFreqsEnum},
        {"clk-domain-rpc", "CLK_DOMAIN RPC", sizeof(NV_GPU_CLOCK_CLK_DOMAIN_RPC), NV_GPU_CLOCK_CLK_DOMAIN_RPC_VER, false, CallClockClkDomainRpc, PrepareClockDomainRpc},
        {"clk-progs-info", "CLK_PROG info", sizeof(NV_GPU_CLOCK_CLK_PROGS_INFO), NV_GPU_CLOCK_CLK_PROGS_INFO_VER, false, CallClockClkProgsGetInfo, nullptr},
        {"clk-progs-status", "CLK_PROG status", sizeof(NV_GPU_CLOCK_CLK_PROGS_STATUS), NV_GPU_CLOCK_CLK_PROGS_STATUS_VER, false, CallClockClkProgsGetStatus, nullptr},
        {"clk-progs-control", "CLK_PROG control", sizeof(NV_GPU_CLOCK_CLK_PROGS_CONTROL), NV_GPU_CLOCK_CLK_PROGS_CONTROL_VER, false, CallClockClkProgsGetControl, nullptr},
        {"clk-progs-set", "CLK_PROG set control", sizeof(NV_GPU_CLOCK_CLK_PROGS_CONTROL), NV_GPU_CLOCK_CLK_PROGS_CONTROL_VER, true, CallClockClkProgsSetControl, nullptr},
        {"clk-enums-info", "CLK enums info", sizeof(NV_GPU_CLOCK_CLK_ENUMS_INFO), NV_GPU_CLOCK_CLK_ENUMS_INFO_VER, false, CallClockClkEnumsGetInfo, nullptr},
        {"clk-vf-rels-info", "CLK VF relationships info", sizeof(NV_GPU_CLOCK_CLK_VF_RELS_INFO), NV_GPU_CLOCK_CLK_VF_RELS_INFO_VER, false, CallClockClkVfRelsGetInfo, nullptr},
        {"clk-vf-rels-status", "CLK VF relationships status", sizeof(NV_GPU_CLOCK_CLK_VF_RELS_STATUS), NV_GPU_CLOCK_CLK_VF_RELS_STATUS_VER, false, CallClockClkVfRelsGetStatus, nullptr},
        {"clk-vf-rels-control", "CLK VF relationships control", sizeof(NV_GPU_CLOCK_CLK_VF_RELS_CONTROL), NV_GPU_CLOCK_CLK_VF_RELS_CONTROL_VER, false, CallClockClkVfRelsGetControl, nullptr},
        {"clk-vf-rels-set", "CLK VF relationships set control", sizeof(NV_GPU_CLOCK_CLK_VF_RELS_CONTROL), NV_GPU_CLOCK_CLK_VF_RELS_CONTROL_VER, true, CallClockClkVfRelsSetControl, nullptr},
        {"clk-vf-points-info", "CLK VF points info", sizeof(NV_GPU_CLOCK_CLK_VF_POINTS_INFO), NV_GPU_CLOCK_CLK_VF_POINTS_INFO_VER, false, CallClockClkVfPointsGetInfo, nullptr},
        {"clk-vf-points-status", "CLK VF points status", sizeof(NV_GPU_CLOCK_CLK_VF_POINTS_STATUS), NV_GPU_CLOCK_CLK_VF_POINTS_STATUS_VER, false, CallClockClkVfPointsGetStatus, nullptr},
        {"clk-vf-points-control", "CLK VF points control", sizeof(NV_GPU_CLOCK_CLK_VF_POINTS_CONTROL), NV_GPU_CLOCK_CLK_VF_POINTS_CONTROL_VER, false, CallClockClkVfPointsGetControl, nullptr},
        {"clk-vf-points-set", "CLK VF points set control", sizeof(NV_GPU_CLOCK_CLK_VF_POINTS_CONTROL), NV_GPU_CLOCK_CLK_VF_POINTS_CONTROL_VER, true, CallClockClkVfPointsSetControl, nullptr},
        {"clk-prop-regimes-info", "CLK_PROP regimes info", sizeof(NV_GPU_CLOCK_CLK_PROP_REGIMES_INFO), NV_GPU_CLOCK_CLK_PROP_REGIMES_INFO_VER, false, CallClockClkPropRegimesGetInfo, nullptr},
        {"clk-prop-regimes-control", "CLK_PROP regimes control", sizeof(NV_GPU_CLOCK_CLK_PROP_REGIMES_CONTROL), NV_GPU_CLOCK_CLK_PROP_REGIMES_CONTROL_VER, false, CallClockClkPropRegimesGetControl, nullptr},
        {"clk-prop-regimes-set", "CLK_PROP regimes set control", sizeof(NV_GPU_CLOCK_CLK_PROP_REGIMES_CONTROL), NV_GPU_CLOCK_CLK_PROP_REGIMES_CONTROL_VER, true, CallClockClkPropRegimesSetControl, nullptr},
        {"clk-prop-tops-info", "CLK_PROP tops info", sizeof(NV_GPU_CLOCK_CLK_PROP_TOPS_INFO), NV_GPU_CLOCK_CLK_PROP_TOPS_INFO_VER, false, CallClockClkPropTopsGetInfo, nullptr},
        {"clk-prop-tops-status", "CLK_PROP tops status", sizeof(NV_GPU_CLOCK_CLK_PROP_TOPS_STATUS), NV_GPU_CLOCK_CLK_PROP_TOPS_STATUS_VER, false, CallClockClkPropTopsGetStatus, nullptr},
        {"clk-prop-tops-control", "CLK_PROP tops control", sizeof(NV_GPU_CLOCK_CLK_PROP_TOPS_CONTROL), NV_GPU_CLOCK_CLK_PROP_TOPS_CONTROL_VER, false, CallClockClkPropTopsGetControl, nullptr},
        {"clk-prop-tops-set", "CLK_PROP tops set control", sizeof(NV_GPU_CLOCK_CLK_PROP_TOPS_CONTROL), NV_GPU_CLOCK_CLK_PROP_TOPS_CONTROL_VER, true, CallClockClkPropTopsSetControl, nullptr},
        {"clk-prop-top-rels-info", "CLK_PROP top relationships info", sizeof(NV_GPU_CLOCK_CLK_PROP_TOP_RELS_INFO), NV_GPU_CLOCK_CLK_PROP_TOP_RELS_INFO_VER, false, CallClockClkPropTopRelsGetInfo, nullptr},
        {"clk-prop-top-rels-control", "CLK_PROP top relationships control", sizeof(NV_GPU_CLOCK_CLK_PROP_TOP_RELS_CONTROL), NV_GPU_CLOCK_CLK_PROP_TOP_RELS_CONTROL_VER, false, CallClockClkPropTopRelsGetControl, nullptr},
        {"clk-prop-top-rels-set", "CLK_PROP top relationships set control", sizeof(NV_GPU_CLOCK_CLK_PROP_TOP_RELS_CONTROL), NV_GPU_CLOCK_CLK_PROP_TOP_RELS_CONTROL_VER, true, CallClockClkPropTopRelsSetControl, nullptr},
        {"adc-devices-info", "ADC devices info", sizeof(NV_GPU_CLOCK_ADC_DEVICES_INFO), NV_GPU_CLOCK_ADC_DEVICES_INFO_VER, false, CallClockAdcDevicesGetInfo, nullptr},
        {"adc-devices-status", "ADC devices status", sizeof(NV_GPU_CLOCK_ADC_DEVICES_STATUS), NV_GPU_CLOCK_ADC_DEVICES_STATUS_VER, false, CallClockAdcDevicesGetStatus, nullptr},
        {"adc-devices-control", "ADC devices control", sizeof(NV_GPU_CLOCK_ADC_DEVICES_CONTROL), NV_GPU_CLOCK_ADC_DEVICES_CONTROL_VER, false, CallClockAdcDevicesGetControl, nullptr},
        {"adc-devices-set", "ADC devices set control", sizeof(NV_GPU_CLOCK_ADC_DEVICES_CONTROL), NV_GPU_CLOCK_ADC_DEVICES_CONTROL_VER, true, CallClockAdcDevicesSetControl, nullptr},
        {"nafll-devices-info", "NAFLL devices info", sizeof(NV_GPU_CLOCK_NAFLL_DEVICES_INFO), NV_GPU_CLOCK_NAFLL_DEVICES_INFO_VER, false, CallClockNafllDevicesGetInfo, nullptr},
        {"nafll-devices-status", "NAFLL devices status", sizeof(NV_GPU_CLOCK_NAFLL_DEVICES_STATUS), NV_GPU_CLOCK_NAFLL_DEVICES_STATUS_VER, false, CallClockNafllDevicesGetStatus, nullptr},
        {"nafll-devices-control", "NAFLL devices control", sizeof(NV_GPU_CLOCK_NAFLL_DEVICES_CONTROL), NV_GPU_CLOCK_NAFLL_DEVICES_CONTROL_VER, false, CallClockNafllDevicesGetControl, nullptr},
        {"nafll-devices-set", "NAFLL devices set control", sizeof(NV_GPU_CLOCK_NAFLL_DEVICES_CONTROL), NV_GPU_CLOCK_NAFLL_DEVICES_CONTROL_VER, true, CallClockNafllDevicesSetControl, nullptr},
        {"clk-freq-controllers-info", "CLK frequency controllers info", sizeof(NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_INFO), NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_INFO_VER, false, CallClockClkFreqControllersGetInfo, nullptr},
        {"clk-freq-controllers-status", "CLK frequency controllers status", sizeof(NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_STATUS), NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_STATUS_VER, false, CallClockClkFreqControllersGetStatus, nullptr},
        {"clk-freq-controllers-control", "CLK frequency controllers control", sizeof(NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_CONTROL), NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_CONTROL_VER, false, CallClockClkFreqControllersGetControl, nullptr},
        {"clk-freq-controllers-set", "CLK frequency controllers set control", sizeof(NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_CONTROL), NV_GPU_CLOCK_CLK_FREQ_CONTROLLERS_CONTROL_VER, true, CallClockClkFreqControllersSetControl, nullptr},
        {"clk-volt-controllers-info", "CLK voltage controllers info", sizeof(NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_INFO), NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_INFO_VER, false, CallClockClkVoltControllersGetInfo, nullptr},
        {"clk-volt-controllers-status", "CLK voltage controllers status", sizeof(NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_STATUS), NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_STATUS_VER, false, CallClockClkVoltControllersGetStatus, nullptr},
        {"clk-volt-controllers-control", "CLK voltage controllers control", sizeof(NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_CONTROL), NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_CONTROL_VER, false, CallClockClkVoltControllersGetControl, nullptr},
        {"clk-volt-controllers-set", "CLK voltage controllers set control", sizeof(NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_CONTROL), NV_GPU_CLOCK_CLK_VOLT_CONTROLLERS_CONTROL_VER, true, CallClockClkVoltControllersSetControl, nullptr},
        {"client-domains-info", "Client CLK domains info", sizeof(NV_GPU_CLOCK_CLIENT_CLK_DOMAINS_INFO), NV_GPU_CLOCK_CLIENT_CLK_DOMAINS_INFO_VER, false, CallClockClientClkDomainsGetInfo, nullptr},
        {"client-vf-info", "Client CLK VF points info", sizeof(NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_INFO), NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_INFO_VER, false, CallClockClientClkVfPointsGetInfo, nullptr},
        {"client-vf-status", "Client CLK VF points status", sizeof(NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_STATUS), NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_STATUS_VER, false, CallClockClientClkVfPointsGetStatus, nullptr},
        {"client-vf-control", "Client CLK VF points control", sizeof(NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_CONTROL), NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_CONTROL_VER, false, CallClockClientClkVfPointsGetControl, nullptr},
        {"client-vf-set", "Client CLK VF points set control", sizeof(NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_CONTROL), NV_GPU_CLOCK_CLIENT_CLK_VF_POINTS_CONTROL_VER, true, CallClockClientClkVfPointsSetControl, nullptr},
        {"pmumon-clk-domains-samples", "PMUMON CLK domain samples", sizeof(NV_GPU_CLOCK_PMUMON_CLK_DOMAINS_GET_SAMPLES), NV_GPU_CLOCK_PMUMON_CLK_DOMAINS_GET_SAMPLES_VER, false, CallClockPmumonClkDomainsGetSamples, nullptr},
    };

    static const ApiSpec kPowerApiSpecs[] =
    {
        {"power-policy-info", "Power policy info", sizeof(NV_GPU_POWER_POLICY_INFO_PARAMS), NV_GPU_POWER_POLICY_INFO_PARAMS_VER, false, CallPowerPolicyGetInfo, nullptr},
        {"power-policy-status", "Power policy status", sizeof(NV_GPU_POWER_POLICY_STATUS_PARAMS), NV_GPU_POWER_POLICY_STATUS_PARAMS_VER, false, CallPowerPolicyGetStatus, nullptr},
        {"power-policy-control", "Power policy control", sizeof(NV_GPU_POWER_POLICY_CONTROL_PARAMS), NV_GPU_POWER_POLICY_CONTROL_PARAMS_VER, false, CallPowerPolicyGetControl, nullptr},
        {"power-policy-set", "Power policy set control", sizeof(NV_GPU_POWER_POLICY_CONTROL_PARAMS), NV_GPU_POWER_POLICY_CONTROL_PARAMS_VER, true, CallPowerPolicySetControl, nullptr},
        {"client-power-topology-info", "Client power topology info", sizeof(NV_GPU_CLIENT_POWER_TOPOLOGY_INFO), NV_GPU_CLIENT_POWER_TOPOLOGY_INFO_VER, false, CallClientPowerTopologyGetInfo, nullptr},
        {"client-power-topology-status", "Client power topology status", sizeof(NV_GPU_CLIENT_POWER_TOPOLOGY_STATUS), NV_GPU_CLIENT_POWER_TOPOLOGY_STATUS_VER, false, CallClientPowerTopologyGetStatus, nullptr},
        {"client-power-policies-info", "Client power policies info", sizeof(NV_GPU_CLIENT_POWER_POLICIES_INFO), NV_GPU_CLIENT_POWER_POLICIES_INFO_VER, false, CallClientPowerPoliciesGetInfo, nullptr},
        {"client-power-policies-status", "Client power policies status", sizeof(NV_GPU_CLIENT_POWER_POLICIES_STATUS), NV_GPU_CLIENT_POWER_POLICIES_STATUS_VER, false, CallClientPowerPoliciesGetStatus, nullptr},
        {"client-power-policies-set", "Client power policies set status", sizeof(NV_GPU_CLIENT_POWER_POLICIES_STATUS), NV_GPU_CLIENT_POWER_POLICIES_STATUS_VER, true, CallClientPowerPoliciesSetStatus, nullptr},
        {"client-thermal-policies-info", "Client thermal policies info", sizeof(NV_GPU_CLIENT_THERMAL_POLICIES_INFO), NV_GPU_CLIENT_THERMAL_POLICIES_INFO_VER, false, CallClientThermalPoliciesGetInfo, nullptr},
        {"client-thermal-policies-status", "Client thermal policies status", sizeof(NV_GPU_CLIENT_THERMAL_POLICIES_STATUS), NV_GPU_CLIENT_THERMAL_POLICIES_STATUS_VER, false, CallClientThermalPoliciesGetStatus, nullptr},
        {"client-thermal-policies-set", "Client thermal policies set status", sizeof(NV_GPU_CLIENT_THERMAL_POLICIES_STATUS), NV_GPU_CLIENT_THERMAL_POLICIES_STATUS_VER, true, CallClientThermalPoliciesSetStatus, nullptr},
        {"perf-policies-info", "Perf policies info", sizeof(NV_GPU_PERF_POLICIES_INFO_PARAMS), NV_GPU_PERF_POLICIES_INFO_PARAMS_VER, false, CallPerfPoliciesGetInfo, nullptr},
        {"perf-policies-status", "Perf policies status", sizeof(NV_GPU_PERF_POLICIES_STATUS_PARAMS), NV_GPU_PERF_POLICIES_STATUS_PARAMS_VER, false, CallPerfPoliciesGetStatus, nullptr},
        {"perf-policies-samples", "Perf policy PMUMON samples", sizeof(NV_GPU_PERF_PMUMON_PERF_POLICIES_GET_SAMPLES), NV_GPU_PERF_PMUMON_PERF_POLICIES_GET_SAMPLES_VER, false, CallPerfPmumonPerfPoliciesGetSamples, nullptr},
    };

    static const ApiSpec kThermalApiSpecs[] =
    {
        {"fan-arbiter-info", "Fan arbiter info", sizeof(NV_GPU_FAN_ARBITER_INFO_PARAMS), NV_GPU_FAN_ARBITER_INFO_PARAMS_VER, false, CallFanArbiterGetInfo, nullptr},
        {"fan-arbiter-status", "Fan arbiter status", sizeof(NV_GPU_FAN_ARBITER_STATUS_PARAMS), NV_GPU_FAN_ARBITER_STATUS_PARAMS_VER, false, CallFanArbiterGetStatus, nullptr},
        {"fan-cooler-info", "Fan cooler info", sizeof(NV_GPU_FAN_COOLER_INFO_PARAMS), NV_GPU_FAN_COOLER_INFO_PARAMS_VER, false, CallFanCoolerGetInfo, nullptr},
        {"fan-cooler-status", "Fan cooler status", sizeof(NV_GPU_FAN_COOLER_STATUS_PARAMS), NV_GPU_FAN_COOLER_STATUS_PARAMS_VER, false, CallFanCoolerGetStatus, nullptr},
        {"fan-cooler-control", "Fan cooler control", sizeof(NV_GPU_FAN_COOLER_CONTROL_PARAMS), NV_GPU_FAN_COOLER_CONTROL_PARAMS_VER, false, CallFanCoolerGetControl, nullptr},
        {"fan-cooler-set", "Fan cooler set control", sizeof(NV_GPU_FAN_COOLER_CONTROL_PARAMS), NV_GPU_FAN_COOLER_CONTROL_PARAMS_VER, true, CallFanCoolerSetControl, nullptr},
        {"fan-policy-info", "Fan policy info", sizeof(NV_GPU_FAN_POLICY_INFO_PARAMS), NV_GPU_FAN_POLICY_INFO_PARAMS_VER, false, CallFanPolicyGetInfo, nullptr},
        {"fan-policy-status", "Fan policy status", sizeof(NV_GPU_FAN_POLICY_STATUS_PARAMS), NV_GPU_FAN_POLICY_STATUS_PARAMS_VER, false, CallFanPolicyGetStatus, nullptr},
        {"fan-policy-control", "Fan policy control", sizeof(NV_GPU_FAN_POLICY_CONTROL_PARAMS), NV_GPU_FAN_POLICY_CONTROL_PARAMS_VER, false, CallFanPolicyGetControl, nullptr},
        {"fan-policy-set", "Fan policy set control", sizeof(NV_GPU_FAN_POLICY_CONTROL_PARAMS), NV_GPU_FAN_POLICY_CONTROL_PARAMS_VER, true, CallFanPolicySetControl, nullptr},
        {"fan-test-info", "Fan test info", sizeof(NV_GPU_FAN_TEST_INFO_PARAMS), NV_GPU_FAN_TEST_INFO_PARAMS_VER, false, CallFanTestGetInfo, nullptr},
        {"fan-pmumon-samples", "Fan PMUMON samples", sizeof(NV_GPU_FAN_PMUMON_FAN_COOLERS_GET_SAMPLES), NV_GPU_FAN_PMUMON_FAN_COOLERS_GET_SAMPLES_VER, false, CallFanPmumonFanCoolersGetSamples, nullptr},
        {"thermal-policy-info", "Thermal policy info", sizeof(NV_GPU_THERMAL_POLICY_INFO_PARAMS), NV_GPU_THERMAL_POLICY_INFO_PARAMS_VER, false, CallThermalPolicyGetInfo, nullptr},
        {"thermal-policy-status", "Thermal policy status", sizeof(NV_GPU_THERMAL_POLICY_STATUS_PARAMS), NV_GPU_THERMAL_POLICY_STATUS_PARAMS_VER, false, CallThermalPolicyGetStatus, nullptr},
        {"thermal-policy-control", "Thermal policy control", sizeof(NV_GPU_THERMAL_POLICY_CONTROL_PARAMS), NV_GPU_THERMAL_POLICY_CONTROL_PARAMS_VER, false, CallThermalPolicyGetControl, nullptr},
        {"thermal-policy-set", "Thermal policy set control", sizeof(NV_GPU_THERMAL_POLICY_CONTROL_PARAMS), NV_GPU_THERMAL_POLICY_CONTROL_PARAMS_VER, true, CallThermalPolicySetControl, nullptr},
        {"therm-channel-info", "Thermal channel info", sizeof(NV_GPU_THERMAL_THERM_CHANNEL_INFO_PARAMS), NV_GPU_THERMAL_THERM_CHANNEL_INFO_PARAMS_VER, false, CallThermChannelGetInfo, nullptr},
        {"therm-channel-status", "Thermal channel status", sizeof(NV_GPU_THERMAL_THERM_CHANNEL_STATUS_PARAMS), NV_GPU_THERMAL_THERM_CHANNEL_STATUS_PARAMS_VER, false, CallThermChannelGetStatus, nullptr},
        {"therm-channel-control", "Thermal channel control", sizeof(NV_GPU_THERMAL_THERM_CHANNEL_CONTROL_PARAMS), NV_GPU_THERMAL_THERM_CHANNEL_CONTROL_PARAMS_VER, false, CallThermChannelGetControl, nullptr},
        {"therm-channel-set", "Thermal channel set control", sizeof(NV_GPU_THERMAL_THERM_CHANNEL_CONTROL_PARAMS), NV_GPU_THERMAL_THERM_CHANNEL_CONTROL_PARAMS_VER, true, CallThermChannelSetControl, nullptr},
        {"therm-device-info", "Thermal device info", sizeof(NV_GPU_THERMAL_THERM_DEVICE_INFO_PARAMS), NV_GPU_THERMAL_THERM_DEVICE_INFO_PARAMS_VER, false, CallThermDeviceGetInfo, nullptr},
        {"therm-monitors-info", "Thermal monitors info", sizeof(NV_GPU_THERMAL_THERM_MONITORS_INFO), NV_GPU_THERMAL_THERM_MONITORS_INFO_VER, false, CallThermMonitorsGetInfo, nullptr},
        {"therm-monitors-status", "Thermal monitors status", sizeof(NV_GPU_THERMAL_THERM_MONITORS_STATUS), NV_GPU_THERMAL_THERM_MONITORS_STATUS_VER, false, CallThermMonitorsGetStatus, nullptr},
        {"therm-hwfs-slowdown", "Thermal HWFS slowdown amount", sizeof(NV_GPU_THERMAL_HWFS_SLOWDOWN_AMOUNT_GET_PARAMS), NV_GPU_THERMAL_HWFS_SLOWDOWN_AMOUNT_GET_PARAMS_VER, false, CallThermHwFsSlowdownAmountGet, nullptr},
        {"thermal-hwfs-info", "Thermal HWFS event info", sizeof(NV_GPU_THERMAL_HWFS_EVENT_SETTINGS_PARAMS), NV_GPU_THERMAL_HWFS_EVENT_SETTINGS_PARAMS_VER, false, CallThermalHwFsGetInfo, nullptr},
        {"thermal-hwfs-set", "Thermal HWFS event set", sizeof(NV_GPU_THERMAL_HWFS_EVENT_SETTINGS_PARAMS), NV_GPU_THERMAL_HWFS_EVENT_SETTINGS_PARAMS_VER, true, CallThermalHwFsSetInfo, nullptr},
        {"thermal-pmumon-samples", "Thermal PMUMON samples", sizeof(NV_GPU_THERMAL_PMUMON_THERM_CHANNELS_GET_SAMPLES), NV_GPU_THERMAL_PMUMON_THERM_CHANNELS_GET_SAMPLES_VER, false, CallThermalPmumonThermChannelsGetSamples, nullptr},
    };
    ...
    static int RunRawApiCommand(const char *groupLabel, const ApiSpec *specs, size_t count, int argc, char **argv)
    {
        ...
        RawApiArgs args;
        if (!ParseRawApiArgs(argc - 1, argv + 1, &args))
        {
            return 1;
        }
        ...
        if (!CollectGpus(args.hasIndex, args.index, handles, indices))
        {
            return 1;
        }
        ...
        for (size_t i = 0; i < handles.size(); ++i)
        {
            ...
            if (args.inPath && !ReadBinaryFile(args.inPath, buffer.get(), spec->size))
            {
                return 1;
            }
            ...
            *reinterpret_cast<NvU32 *>(buffer.get()) = spec->version;
            ...
            if (args.outPath)
            {
                ...
                if (!WriteBinaryFile(outPath.c_str(), buffer.get(), spec->size))
                {
                    return 1;
                }
    ...
    int CmdGpuClock(int argc, char **argv)
    {
        ...
        {
            return RunRawApiCommand("clock", kClockApiSpecs, sizeof(kClockApiSpecs) / sizeof(kClockApiSpecs[0]), argc - 1, argv + 1);
        }
    ...
    int CmdGpuThermalApi(int argc, char **argv)
    {
        ...
        return RunRawApiCommand("thermal", kThermalApiSpecs, sizeof(kThermalApiSpecs) / sizeof(kThermalApiSpecs[0]), argc, argv);
    }
}
