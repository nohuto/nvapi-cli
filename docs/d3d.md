...
## d3d latency mark
Uses `NvAPI_D3D_SetLatencyMarker` (`NV_LATENCY_MARKER_PARAMS`) to insert a latency marker for a frame ID. Marker names accept aliases.

```c
--frame ID // monotonically increasing frame ID
--type TYPE // sim-start|sim-end|render-start|render-end|present-start|present-end|input|flash|ping
// aliases: simulation-start/end, rendersubmit-start/end, input-sample, trigger-flash, pc-latency-ping
```
...
--feature NAME:STATE[:VK] // NAME=bw|blackwhite|black-and-white|hudless, STATE=enable|disable, optional per-feature VK
```
